...
## Last updated: 2026-06-27 (Session 78 — Messenger monochrome redesign + one-to-one voice calling live with self-hosted TURN; video removed. See SESSION 78 block below.)

## SESSION 78 UPDATE (June 27, 2026) — Messenger redesign, voice calling (coturn TURN), video removed

These are live and verified on kiko.vanhawke.agency.

**MESSENGER (/messages) — internal team messaging.** Direct messages (Sunny ↔ Matt) and group channels (General, Alpine Campaign) with @kiko bot replies, file/image share (vela-assets bucket), presence (online/away/offline), read receipts, reactions, threaded replies, typing indicators, edit/delete/pin/forward, and mute. Real-time via Supabase Realtime (kiko_team_messages + kiko_user_presence in the publication). Backed by api/team-messages.js (server.js routes /api/team-messages, requireAuth global). Sunny's messaging data is owner-scoped — Matt cannot see Sunny's DMs or private channel content.

**ONE-TO-ONE VOICE CALLING (audio only).** WebRTC peer-to-peer audio in the Messenger (src/hooks/useVoiceCall.js, Supabase Realtime signaling). A self-hosted coturn TURN relay runs on the Hetzner box (178.104.73.22:3478 udp/tcp, relay ports 49200-49250/udp, realm vanhawke.agency) so calls connect across NAT/firewalls — STUN-only previously failed on many networks. The frontend fetches EPHEMERAL HMAC TURN credentials from an authenticated endpoint /api/team-messages?action=turn-credentials (TURN_SECRET lives only in the worker .env; credentials are time-limited, cached until ~5min before expiry, and fall back to STUN-only on failure). Call controls: mute, end.

**VIDEO REMOVED (not just hidden).** No camera is ever requested — getMedia is audio-only. The in-call video elements and the toggle-video and screen-share buttons were removed. Calls are audio-only by design.

**MESSENGER UI — monochrome iMessage/WhatsApp style.** Aligned bubbles: own messages right in near-black (#1d1d20, white text), others left in light grey (#F0F0F2), greyscale charcoal avatars, no in-thread avatars (sender label + greyscale AI badge for Kiko only), monochrome "· Read" receipt, rounded composer with a dark circular send button. Header is voice-call + search + files (no video). Built in src/pages/Messages.jsx (local C theme: accent flipped to near-black, avatars neutralised to charcoal).
...
**1. THE TWO-WAY-TOUCH DOCTRINE (what earns a task).** A task is earned by a real prior touch, and a touch is a REPLY received — an inbound email or a phone conversation — or a greenlit campaign enrolment. An email SENT with no reply (personal OR campaign) is NOT a touch and NOT a relationship; sending into silence is cold. Warm vs cold is two-level: a contact-level reply drives follow-up vs first-outreach; a company-level reply drives warm vs cold. A company that has replied + a contact who has not = a legitimate first outreach (a real task). Neither has replied = PARK it. This is enforced in code (`companyHasReply` gate in kiko-tools.js + cron-event-processor.js). The ADAPTIVE FOLLOW-UP SCAN (monitors/follow-up-monitor.js + api/lib/followup-cadence.js, runs 8am Doha weekday mornings) applies this same doctrine to follow-up timing: only a genuine inbound human reply earns a card (OOO/auto-replies filtered; campaign-only-no-reply stays with the sequence sender), and cadence is a recency-weighted read on the person — median reply latency (90-day half-life), temperature (HOT/WARM/COOLING/GOING_COLD, opens excluded), seniority floor supreme — never a flat timer. It writes ONE approval card per contact into kiko_draft_actions behind the hard approval gate (nothing sends without Sunny's tap); awaiting_us threads surface promptly as respond cards, awaiting_them are cadence-paced, closing never cards; warm-gone-quiet threads (>21d or overdue beyond one interval) park visibly into kiko_parked_intelligence as re_engagement. Scan v2 hardening: each card is owned by the thread's SENDER (routed from kiko_email_tracking.user_id, fail-closed on a null owner) so Matt's follow-ups land on Matt's surface and Sunny's never surface to Matt (the Today view filters cards to the current user; Sunny sees Matt's only via the super-admin oversight path, never by absorbing them); intent is deterministic (an ambiguous reply is classified once by Haiku at temperature 0 and cached by a fingerprint of the exact reply text in kiko_intent_cache, so a thread can never flip card<->park between runs); the drafted message is frozen once written (only the day-count and cost line refresh); a thread that turns closing or ages out self-retires its pending card (status superseded); and card titles use real names/companies from kiko_relationships, which the email monitor auto-enriches on every inbound reply (it pulls the sender's real display name from the From header and derives company from the domain, gap-fill only so a curated name is never clobbered, scoped to the inbox owner's user_id, no model tokens). On the Today view these cards render as compact ONE-LINERS folded INTO Tasks Due ("Reply to X — Co" for respond, "Chase X — Co (no reply, Nd)" for chase), full draft on tap.
...
- Pipeline health (30min), Email replies (2min), Follow-up scan (8am Doha weekday mornings: adaptive cadence -> approval cards), Scheduled sender (5min)
