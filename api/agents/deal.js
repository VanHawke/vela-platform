...
export async function callDealAgent(instruction, userEmail = 'sunny@vanhawke.com', userId = null) {
  try {
    ...
    const response = await anthropic.messages.create({
      model: 'claude-sonnet-4-6-20250514', // Upgraded from Haiku — deal analysis needs real reasoning
      max_tokens: 300,
  ...
  if (['Won', 'Lost', 'won', 'lost'].includes(new_stage)) {
    try {
      ...
      const analysis = await new Anthropic({ apiKey: process.env.ANTHROPIC_KEY }).messages.create({
        model: 'claude-sonnet-4-6-20250514', // Upgraded from Haiku — deal analysis needs real reasoning max_tokens: 400,
