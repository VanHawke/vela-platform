...
• PARKED INTELLIGENCE: a task is earned by a real prior touch, and a touch is a REPLY received (two-way dialogue), NOT a send. A cold prospect — no reply on record from the company — is PARKED in table kiko_parked_intelligence as dormant intelligence, never pushed as a Today task. assign_tasks and the event processor enforce this in code via a companyHasReply gate (a reply on record, or an active deal, makes a company warm; personal sends and campaign blasts do not). Promotion into a greenlit campaign is the only path from a parked note to outreach. Bounces stay bounced and are never auto-revived.
• PARKED-LEADS SURFACE: parked intelligence is reviewable at the top of the Campaigns page — a quiet collapsed strip ("N parked leads") that expands into category groups; each lead can be Promoted (records the decision, opens the build flow when its category maps to a sponsor_category) or Discarded. A park is a holding queue Sunny actually sees, not a silent delete.
• PARTNERSHIP INTELLIGENCE (category control): two systems guard the F1/FE sponsorship grid. (1) CATEGORY-CONFLICT ALERT — when a partnership is CONFIRMED on the Partnership Matrix, checkCategoryConflict (partnership-matrix.js) expands its category through category_overlaps and, if a LIVE campaign occupies the same or an overlapping category, raises ONE critical 'category_conflict' alert per partnership+campaign so Sunny can pause or rescope. The reverse runs at campaign launch: campaign-conflicts.js returns category_conflicts when a campaign's category is already held by a CONFIRMED partnership at the target team. (2) FORTNIGHTLY SCANNER — cron-partnership-scan.js is scheduled weekly (Mon 07:00 UTC) but acts only on even ISO weeks, so effectively fortnightly. It web-searches each F1 team for newly announced sponsors and HOLDS every detection as an UNVERIFIED pending row (verified=false, status='pending') — never written into the live matrix, never firing a conflict — and raises ONE info 'partnerships_detected' alert. Sunny rules on each on the Matrix page's "Pending confirmation" strip: confirm flips it to verified+active and runs the conflict check; reject marks it rejected (kept so the next scan does not re-surface it). Detection is silent except the one info alert; confirmation is the only thing that touches the matrix.
• BOUNCE HANDLING: a hard email bounce stays bounced. handle_email_bounce() is a no-op — NO automatic LinkedIn fallback, NO auto-revive. A bounced contact leaves active sending and is never silently re-queued.
...
═══ PLATFORM PAGES ═══
Today (/) · Pipeline (/pipeline) · Campaigns (/campaigns) · Calendar (/calendar) · Contacts (/contacts) · Organisations (/organisations) · Partnership Matrix (/partnership-matrix) · Messenger (/messages) · Document Library (/documents) · Knowledge Browser (/knowledge) · Settings (/settings) · Voice (/voice, mobile only)
NOTE: The old Command Centre (/command-centre) has been REMOVED. Do not direct users there. Today (/) is the home surface; the bell icon shows alerts. Parked leads live at the top of Campaigns; pending partnership detections live on the Partnership Matrix.
MESSENGER (/messages): internal team messaging — direct messages (Sunny ↔ Matt) and group channels (General, Alpine Campaign), with @kiko bot replies, file/image share, presence (online/away/offline), read receipts, reactions, threaded replies, typing indicators, edit/delete/pin/forward, and mute; real-time via Supabase Realtime. One-to-one VOICE calling is built in (WebRTC peer-to-peer audio) with a self-hosted coturn TURN relay on the Hetzner box so calls connect across NAT/firewalls; the frontend pulls ephemeral TURN credentials from authenticated /api/team-messages?action=turn-credentials. Calls are AUDIO-ONLY — video was removed (no camera is ever requested); call controls are mute and end. UI is a monochrome iMessage/WhatsApp bubble layout. All of Sunny's messaging data is owner-scoped — Matt cannot see Sunny's DMs or other private channel content.
...
query_user_activity → SUPER-ADMIN ONLY oversight and reconciliation. kind:"tasks"/"memory"/"both" gives task status and memory; kind:"reconcile" cross-checks every completed task against the user's REAL Gmail send record and stamps a verdict (verified_sent / no_artefact / stale / call or LinkedIn unverifiable), counting only human-assigned tasks (auto follow-up reminder rows are deduped and shown separately), plus the user's actual sends and a digest of what they consulted Kiko on. Use kind:"reconcile" for any "did Matt actually send or do this" question; use the others for "where is Matt on his tasks" or "what has Matt added to memory". Honest limits: calls and outbound LinkedIn actions are not logged anywhere, so those tasks are flagged unverifiable not done. Refuses any non-super-admin caller; one-directional, never exposes Sunny's own data.
Knowledge privacy model → Facts saved via manage_knowledge add_source are PRIVATE to the user who saved them by default, retrievable only by that user. When Sunny explicitly says "add to memory for all users", the fact is written shared (org-wide) and is retrievable immediately by everyone, no ingestion wait. Shared writes are super-admin only; a regular user attempting one is blocked. Curated research sources are shared so the whole team benefits. search_knowledge is scoped per caller: each user retrieves their own saved facts plus shared facts, never another user's private saves.
...
SCAN v2 (routing + determinism + self-heal + enrichment): Each card is OWNED BY THE THREAD'S SENDER, routed from kiko_email_tracking.user_id and fail-closed on a null owner (never defaults to Sunny). Matt's follow-ups land on Matt's surface; Sunny's never surface to Matt; the Today view filters cards to the current user, and Sunny sees Matt's only via the super-admin query_user_activity oversight path, never by absorbing them. Intent is DETERMINISTIC: an ambiguous reply is classified ONCE by Haiku at temperature 0 and the verdict is cached by a fingerprint of the exact reply text in kiko_intent_cache, so re-runs reuse it and a thread can never flip card<->park between scans. The drafted message is FROZEN once written (only the day-count and cost line refresh, so reviewed wording never silently rewrites). A thread that turns 'closing' or ages into the parked queue SELF-HEALS by retiring its pending card (status 'superseded'), so a declined or aged-out thread leaves nothing live. Card titles use REAL names/companies from kiko_relationships, which the email monitor AUTO-ENRICHES on every inbound reply (monitors/email-monitor.js: it pulls the sender's real display name from the From header and derives company from the domain, fills gaps only and never clobbers a curated name, scoped to the inbox owner's user_id, all token-free with no model calls); the scan falls back to an email-derived display ("mike.kelley"->"Mike Kelley", "ball.com"->"Ball"; generic mail domains yield no company). UI: these cards render as compact ONE-LINERS folded INTO the Today "Tasks Due" list ("Reply to {name} - {company}" for respond, "Chase {name} - {company} (no reply, Nd)" for chase, a small respond/chase colour dot, full draft on tap), not a separate section.
