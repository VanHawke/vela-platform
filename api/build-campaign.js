  ...
  try {
    ...
    if (!sequenceId) {
      ...
      const { data: newSeq, error: seqErr } = await supabase.from('kiko_sequences').insert({
        ...
        is_active: false,
        steps: draftedSteps,
        metadata: { category_id: category, team_id: team.id },
      }).select().single();
    ...
    // -- Web sourcing: SPLIT into (1) research w/ web_search, (2) structure to JSON --
    // web_search is token-intensive; its per-iteration reasoning competes with the final
    // JSON for the same max_tokens. Bundling them truncated the JSON (worst for CRM-rich
    // categories: long exclusion list + 20 companies) -> zero web companies. So research
    // gathers freely (compact text, capped searches); a tool-free call then emits clean JSON.
    const researchPrompt = `You are sourcing ${webGap} companies in the ${catRow.name} sector for an F1 sponsorship campaign with ${team.name}. This is a real outreach list.

CONTEXT: ${crmCompanyCount} ${catRow.name} companies are ALREADY in our CRM and added separately. Find FRESH companies NOT in our CRM and NOT already F1 partners.
...
EXCLUSION — do NOT include any of these (already F1 partners or already in our CRM): ${exclusionListForPrompt}

Use web search to find real companies that fit. For EACH company, output on its own lines:
COMPANY: <name>
DOMAIN: <real website domain, e.g. paulweiss.com — never invented>
REVENUE: <estimate>
HQ: <city, country>
RATIONALE: <one sentence on sponsorship fit>
DECISION_MAKERS: <Name — Title; Name — Title>  (2-3 real sponsorship-relevant execs: CMO, VP Marketing, Head of Brand/Sponsorship, CRO, CEO, CFO, Head of BD)
---
Find ${webGap} companies. Be concise — no preamble, no closing summary.`;
    ...
    const researchText = researchRes.content.filter(b => b.type === 'text').map(b => b.text).join('').trim();

    // (2) STRUCTURE the research into strict JSON — NO tools, full token budget
    let sourced = [];
    if (researchText) {
      try {
        const structureRes = await anthropic.messages.create({
          model: 'claude-sonnet-4-6',
          max_tokens: 16000,
          messages: [{ role: 'user', content: `Convert the company research below into a JSON array. Return ONLY the array — no markdown fences, no explanation.

Each entry must have exactly: company, domain, revenue, hq, rationale, decision_makers (array of {name, title}).
Skip any company without a real domain. Never invent companies or email addresses.
...
JSON array:` }],
        ...
        const structureText = structureRes.content.filter(b => b.type === 'text').map(b => b.text).join('');
        const cleaned = structureText.replace(/```json|```/g, '').trim();
        const match = cleaned.match(/\[[\s\S]*\]/);
        if (match) sourced = JSON.parse(match[0]);
      } catch (err) {
        console.error('[build-campaign] structure-to-JSON failed:', err.message);
      }
    }
    if (!Array.isArray(sourced)) sourced = [];
    console.log(`[build-campaign] web sourcing — research chars=${researchText.length}, structured companies=${sourced.length}`);
