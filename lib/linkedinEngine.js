...
async function openContextForIdentity(identity) {
  ...
  console.log(`[linkedinEngine] Opening context for identity: ${identity}, proxy: ${process.env.PROXY_HOST || "NO PROXY"}`);
...
export async function sendConnection(identity, profileUrl, note = null) {
  return withLock(identity, async () => {
    ...
    try {
      ...
      // Strategy 1: Direct "Connect" button on profile (scoped to main content)
      const connectDirect = page.locator('main button:has-text("Connect")').first();
      if (await connectDirect.count() > 0) {
        await connectDirect.evaluate(el => { el.scrollIntoView({ block: 'center' }); });
        await page.waitForTimeout(500);
        await connectDirect.evaluate(el => el.click());
      } else {
        // Strategy 2: Profile overflow "More" button (aria-label, NOT nav bar text)
        const profileMore = page.locator('main button[aria-label*="More"]').first();
        const profileMoreAlt = page.locator('main section button:has-text("More")').first();
        
        let moreClicked = false;
        if (await profileMore.count() > 0) {
          // Use JS click to bypass Playwright visibility checks (nav bar overlap)
          await profileMore.evaluate(el => { el.scrollIntoView({ block: 'center' }); });
          await page.waitForTimeout(500);
          await profileMore.evaluate(el => el.click());
          moreClicked = true;
        } else if (await profileMoreAlt.count() > 0) {
          await profileMoreAlt.evaluate(el => { el.scrollIntoView({ block: 'center' }); });
          await page.waitForTimeout(500);
          await profileMoreAlt.evaluate(el => el.click());
          moreClicked = true;
        }
        
        if (!moreClicked) {
          // Strategy 3: Fallback — skip first More (nav), click second
          const allMore = page.locator('button:has-text("More")');
          const count = await allMore.count();
          if (count > 1) {
            await allMore.nth(1).click({ force: true, delay: 100 });
            moreClicked = true;
          }
        }
        
        if (!moreClicked) {
          return { ok: false, status: 'already_connected', error: 'no Connect or More button found' };
        }
        
        await page.waitForTimeout(800);
        
        // Look for Connect in the dropdown
        const connectInMenu = page.getByText('Connect', { exact: true }).first();
        if (await connectInMenu.count() === 0) {
          return { ok: false, status: 'blocked', error: 'Connect not in More menu' };
        }
        await connectInMenu.evaluate(el => el.click());
      }
