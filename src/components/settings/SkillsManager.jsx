...
export default function SkillsManager() {
  ...
  const inp = { width: '100%', background: 'rgba(255,255,255,0.03)', border: `1px solid ${T.border}`, borderRadius: 10, padding: '8px 12px', fontSize: 13, color: T.text, fontFamily: T.font, fontWeight: 300, outline: 'none' }

  if (loading) return <div style={{ color: T.textTertiary, fontSize: 13 }}>Loading skills...</div>

  return (
    <div style={{ display: 'flex', flexDirection: 'column', gap: 16 }}>
      <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center' }}>
        <div>
          <h3 style={{ fontSize: 14, fontWeight: 400, color: T.text, margin: 0, fontFamily: T.font }}>Kiko Skills</h3>
          <p style={{ fontSize: 11, color: T.textTertiary, margin: '2px 0 0', fontFamily: T.font }}>{skills.length} skills — auto-injected into Kiko by keyword match</p>
        </div>
        <button onClick={startNew} style={{ padding: '6px 16px', borderRadius: 50, background: T.accentSoft, border: `1px solid ${T.accentBorder}`, color: 'rgba(139,108,246,0.7)', fontSize: 12, cursor: 'pointer', fontFamily: T.font, fontWeight: 400 }}>+ New Skill</button>
      </div>
        ...
        <div style={card}>
          <div style={{ display: 'flex', flexDirection: 'column', gap: 10 }}>
            ...
            <div style={{ display: 'flex', gap: 8, justifyContent: 'flex-end' }}>
              <button onClick={() => setEditing(null)} style={{ padding: '6px 16px', borderRadius: 50, background: 'transparent', border: `1px solid ${T.border}`, color: T.textTertiary, fontSize: 12, cursor: 'pointer', fontFamily: T.font }}>Cancel</button>
              <button onClick={save} disabled={!form.name || !form.content} style={{ padding: '6px 16px', borderRadius: 50, background: T.accentGradient, border: 'none', color: 'rgba(255,255,255,0.9)', fontSize: 12, cursor: 'pointer', fontFamily: T.font, fontWeight: 400, opacity: form.name && form.content ? 1 : 0.4 }}>Save</button>
            </div>
          </div>
        </div>
      )}
