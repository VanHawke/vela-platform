...
export default function SkillsManager() {
  ...
  const inp = { width: '100%', background: 'rgba(255,255,255,0.03)', border: `1px solid ${T.border}`, borderRadius: 10, padding: '8px 12px', fontSize: 14, color: T.text, fontFamily: T.font, fontWeight: 300, outline: 'none' }
        ...
        <div style={card}>
          <div style={{ display: 'flex', flexDirection: 'column', gap: 10 }}>
            ...
            <div style={{ display: 'flex', gap: 8, justifyContent: 'flex-end' }}>
              ...
              <button onClick={save} disabled={!form.name || !form.content} style={{ padding: '6px 16px', borderRadius: 50, background: T.accentGradient, border: 'none', color: 'rgba(255,255,255,0.9)', fontSize: 13, cursor: 'pointer', fontFamily: T.font, fontWeight: 400, opacity: form.name && form.content ? 1 : 0.4 }}>Save</button>
            </div>
          </div>
        </div>
      )}
