...
export default function LoginPage() {
  ...
  const glassInput = {
    ...
    border: '1px solid rgba(255,224,194,0.10)',
    background: 'rgba(255,224,194,0.04)',
  ...
  return (
    <div style={{ display: 'flex', flexDirection: 'column', alignItems: 'center', justifyContent: 'center', width: '100vw', height: '100vh', background: '#111111', fontFamily: T.font, position: 'relative', overflow: 'hidden' }}>
      ...
      <div style={{ position: 'absolute', width: 500, height: 500, borderRadius: '50%', background: 'radial-gradient(circle, rgba(255,224,194,0.06) 0%, transparent 70%)', top: '50%', left: '50%', transform: 'translate(-50%, -65%)', zIndex: 1, pointerEvents: 'none' }} />
      ...
      <div style={{ width: '100%', maxWidth: 380, textAlign: 'center', position: 'relative', zIndex: 5, padding: '0 24px' }}>
        ...
        <div style={{ marginBottom: 64, display: 'flex', justifyContent: 'center', ...fade(0) }}>
          {customLogo ? (
            ...
            <>
              <span style={{ fontSize: 12, fontWeight: 400, color: 'rgba(255,224,194,0.5)', letterSpacing: '0.15em', fontFamily: T.font }}>VAN HAWKE</span>
              <span style={{ fontSize: 9, verticalAlign: 'super', color: 'rgba(255,224,194,0.3)', marginLeft: 2 }}>™</span>
            </>
          )}
        </div>
        ...
        <p style={{ fontSize: 13, color: 'rgba(255,224,194,0.25)', fontWeight: 300, margin: '0 0 48px', letterSpacing: '0.02em', ...fade(0.5) }}>Intelligence, Applied</p>
        ...
        <div style={fade(0.6)}>
          <button onClick={googleLogin} disabled={gLoading} style={{
            ...
            onMouseEnter={e => { e.currentTarget.style.background = 'rgba(255,224,194,0.10)'; e.currentTarget.style.borderColor = 'rgba(255,224,194,0.20)'; e.currentTarget.style.transform = 'translateY(-1px)' }}
            onMouseLeave={e => { e.currentTarget.style.background = 'rgba(255,224,194,0.05)'; e.currentTarget.style.borderColor = 'rgba(255,224,194,0.12)'; e.currentTarget.style.transform = 'translateY(0)' }}
        ...
        <div style={{ marginTop: 16, ...fade(0.7) }}>
          <button onClick={() => setShowEmail(!showEmail)} style={{
            background: 'none', border: 'none', color: 'rgba(255,224,194,0.20)', fontSize: 12, cursor: 'pointer',
            ...
            onMouseLeave={e => e.currentTarget.style.color = 'rgba(255,224,194,0.20)'}
          >
        ...
        <div style={{ overflow: 'hidden', maxHeight: showEmail ? 260 : 0, opacity: showEmail ? 1 : 0, transition: 'max-height 0.4s ease, opacity 0.3s ease', marginTop: showEmail ? 12 : 0 }}>
          <form onSubmit={emailLogin}>
            <input type="email" placeholder="Email" value={email} onChange={e => setEmail(e.target.value)}
              ...
              onFocus={e => e.target.style.borderColor = 'rgba(255,224,194,0.30)'}
              onBlur={e => e.target.style.borderColor = 'rgba(255,224,194,0.10)'} />
            <div style={{ position: 'relative', marginBottom: 10 }}>
              ...
              <button type="button" onClick={() => setShowPw(!showPw)} style={{
                ...
                background: 'none', border: 'none', cursor: 'pointer', color: 'rgba(255,224,194,0.20)', padding: 0,
            ...
            <button type="submit" disabled={loading} style={{
              ...
              background: 'linear-gradient(135deg, rgba(255,224,194,0.55), rgba(57,48,40,0.55))',
              color: 'rgba(238,238,238,0.95)', border: 'none',
              fontSize: 14, fontWeight: 400, cursor: 'pointer', fontFamily: T.font,
              transition: 'all 0.3s',
              boxShadow: '0 4px 24px rgba(255,224,194,0.15)',
        ...
        {/* Footer */}
        <p style={{ fontSize: 11, color: 'rgba(255,224,194,0.15)', marginTop: 48, fontWeight: 300, ...fade(0.8) }}>By Van Hawke</p>
      </div>
    </div>
  )
}