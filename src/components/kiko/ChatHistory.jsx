...
export default function ChatHistory({ user, open, onToggle, onSelectConversation, onNewChat, activeConvId, onShowAllChats }) {
  ...
  const groupedConvos = useMemo(() => {
    ...
    const filtered = searchQuery.trim() ? allConvos.filter(c => (c.title || '').toLowerCase().includes(searchQuery.toLowerCase())) : allConvos
  ...
  const GroupLabel = ({ label }) => <div style={{ padding: '10px 12px 4px', fontSize: 11, fontWeight: 500, color: '#A0A0A0', fontFamily: T.font, letterSpacing: '0.03em' }}>{label}</div>
