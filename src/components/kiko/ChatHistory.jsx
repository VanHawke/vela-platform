...
export default function ChatHistory({ user, open, onToggle, onSelectConversation, onNewChat, activeConvId }) {
  ...
  async function loadAll() {
    ...
    // Load Kiko conversations + imported conversations in parallel
    const [kikoRes, importedRes] = await Promise.all([
      orgId ? supabase.from('conversations').select('id, title, messages, updated_at').eq('org_id', orgId).neq('archived', true).order('updated_at', { ascending: false }).limit(50) : Promise.resolve({ data: [] }),
      supabase.from('kiko_imported_conversations').select('id, title, source, messages, original_date').eq('user_id', user.id).eq('processed', true).order('original_date', { ascending: false }).limit(500),
    ])

    // Normalize Kiko conversations
    const kiko = (kikoRes.data || []).filter(c => !deletedIdsRef.current.has(c.id)).map(c => ({
      id: c.id, title: c.title || 'Untitled', date: c.updated_at,
      messages: c.messages || [], type: 'kiko',
    }))

    // Normalize imported conversations
    const imported = (importedRes.data || []).map(c => ({
      id: 'imp_' + c.id, title: c.title || 'Untitled', date: c.original_date,
      messages: (c.messages || []).map(m => ({ role: m.role === 'human' ? 'user' : m.role, content: m.content || '' })),
      type: 'imported', source: c.source,
    }))

    // Merge and sort by date descending
    const merged = [...kiko, ...imported].sort((a, b) => new Date(b.date || 0) - new Date(a.date || 0))
    setAllConvos(merged)
    setLoading(false)
  }
  ...
  async function deleteConversation(conv) {
    ...
    if (conv.type === 'kiko') {
      ...
      const realId = conv.id.replace('imp_', '')
      setAllConvos(prev => prev.filter(c => c.id !== conv.id))
      await supabase.from('kiko_imported_conversations').delete().eq('id', realId)
    }
  }
  ...
  async function renameConversation(conv) {
    ...
    if (conv.type === 'kiko') {
      ...
      const realId = conv.id.replace('imp_', '')
      await supabase.from('kiko_imported_conversations').update({ title: renameValue.trim() }).eq('id', realId)
    }
  ...
  function getPreview(conv) {
    ...
    const first = msgs.find(m => m.role === 'user' || m.role === 'human')
    if (!first) return msgs[0]?.content?.slice(0, 80) || 'Empty'
    return (first.content || '').slice(0, 80) + ((first.content || '').length > 80 ? '…' : '')
  }
        ...
        <div style={{ flex: 1, overflowY: 'auto', padding: '8px' }}>
          {loading ? (
            ...
            allConvos.map(conv => {
              ...
              return (
                <div key={conv.id}
                  onClick={() => onSelectConversation({ id: conv.id, messages: conv.messages, title: conv.title, type: conv.type })}
                  style={{
                    padding: '12px 14px', borderRadius: 14, cursor: 'pointer', marginBottom: 2,
                ...
                >
                  <div style={{ flex: 1, minWidth: 0 }}>
                    {renamingId === conv.id ? (
                      ...
                      <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: 3 }}>
                        <span style={{ fontSize: 14, fontWeight: 500, color: '#fff', fontFamily: T.font, whiteSpace: 'nowrap', overflow: 'hidden', textOverflow: 'ellipsis', maxWidth: 280 }}>
                    ...
                    <p style={{ fontSize: 13, color: 'rgba(255,255,255,0.4)', fontFamily: T.font, margin: 0, whiteSpace: 'nowrap', overflow: 'hidden', textOverflow: 'ellipsis', lineHeight: 1.4 }}>
                      {msgCount > 0 ? `${msgCount} msgs · ` : ''}{getPreview(conv)}
                    </p>
                  </div>
                  ...
                  <div style={{ position: 'relative', flexShrink: 0, marginTop: 2 }} ref={menuOpenId === conv.id ? menuRef : null}>
                    <button onClick={(e) => { e.stopPropagation(); setMenuOpenId(menuOpenId === conv.id ? null : conv.id) }}
                      style={{ width: 28, height: 28, borderRadius: 8, border: 'none', background: menuOpenId === conv.id ? 'rgba(255,255,255,0.08)' : 'transparent', cursor: 'pointer', display: 'flex', alignItems: 'center', justifyContent: 'center', color: 'rgba(255,255,255,0.3)', transition: 'all 0.15s' }}
                    ...
                    {menuOpenId === conv.id && (
                      <div onClick={e => e.stopPropagation()} style={{ position: 'absolute', right: 0, top: '100%', marginTop: 4, width: 160, background: 'rgba(20,20,26,0.95)', backdropFilter: 'blur(20px)', borderRadius: 14, border: `1.5px solid ${T.glassBorder}`, boxShadow: '0 8px 32px rgba(0,0,0,0.4)', padding: 6, zIndex: 300 }}>
                        <button onClick={() => { setMenuOpenId(null); setRenamingId(conv.id); setRenameValue(conv.title || '') }} style={{ width: '100%', padding: '9px 12px', borderRadius: 8, border: 'none', background: 'transparent', color: '#fff', textAlign: 'left', fontSize: 14, cursor: 'pointer', fontFamily: T.font, display: 'flex', alignItems: 'center', gap: 10, transition: 'background 0.1s' }}
                        ...
                        <button onClick={() => deleteConversation(conv)} style={{ width: '100%', padding: '9px 12px', borderRadius: 8, border: 'none', background: 'transparent', color: '#C62828', textAlign: 'left', fontSize: 14, cursor: 'pointer', fontFamily: T.font, display: 'flex', alignItems: 'center', gap: 10, transition: 'background 0.1s' }}
