  ...
  return (
    <div style={{ flex: 1, display: 'flex', flexDirection: 'column', maxWidth: 700, margin: '0 auto', padding: '40px 24px 24px', width: '100%' }}>
      ...
      <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', marginBottom: 24 }}>
        ...
        <button onClick={onClose} style={{ width: 36, height: 36, borderRadius: 50, border: 'none', background: '#26262f', cursor: 'pointer', display: 'flex', alignItems: 'center', justifyContent: 'center', color: '#9b9ba3', transition: 'background 0.15s' }}
          onMouseOver={e => e.currentTarget.style.background = '#26262f'}
          onMouseOut={e => e.currentTarget.style.background = '#26262f'}>
          <X size={18} />
        </button>
      </div>
      ...
      <div style={{ position: 'relative', marginBottom: 20 }}>
        <Search size={16} style={{ position: 'absolute', left: 14, top: '50%', transform: 'translateY(-50%)', color: '#7e7e88' }} />
      ...
      {/* Count */}
      <div style={{ fontSize: 12, color: '#7e7e88', fontFamily: T.font, marginBottom: 12, padding: '0 4px' }}>
      ...
      <div style={{ flex: 1, overflowY: 'auto' }}>
        {!searching && displayed.length === 0 ? (
          <p style={{ textAlign: 'center', padding: 40, color: '#7e7e88', fontSize: 14, fontFamily: T.font }}>
            {search ? 'No chats match your search' : 'No conversations yet'}
          </p>
        ) : (
          displayed.map(conv => (
            <div key={conv.id} onClick={() => { onSelect(conv); onClose() }}
              style={{ padding: '14px 16px', borderBottom: `1px solid rgba(124,92,252,0.04)`, cursor: 'pointer', display: 'flex', alignItems: 'center', justifyContent: 'space-between', transition: 'background 0.1s', borderRadius: 8 }}
              ...
              <div style={{ flex: 1, minWidth: 0 }}>
                ...
                <div style={{ display: 'flex', alignItems: 'center', gap: 8, marginTop: 2 }}>
                  <span style={{ fontSize: 12, color: '#7e7e88', fontFamily: T.font }}>{conv.date ? timeAgo(conv.date) : ''}</span>
                  {conv.matchType === 'content' && search && (
                    <span style={{ fontSize: 10, padding: '1px 6px', borderRadius: 50, background: 'rgba(124,92,252,0.12)', color: 'rgba(124,92,252,0.7)', fontFamily: T.font }}>in messages</span>
                  )}
                </div>
              </div>
