...
export function InsightsBadge({ count, onClick }) {
  ...
  return (
    ...
    >
      ...
      <span style={{ fontSize: 12, color: '#9b9ba3', fontWeight: 400 }}>
        {count} alert{count !== 1 ? 's' : ''}
      </span>
      <ChevronRight size={10} style={{ color: '#56565e' }} />
    </button>
  )
}
  ...
  return (
    <>
      ...
      <div style={{
        ...
        background: '#1c1c24', borderLeft: `1px solid ${T.border}`,
        ...
        <div style={{ padding: '14px 16px', borderBottom: `1px solid ${T.border}`, display: 'flex', alignItems: 'center', justifyContent: 'space-between' }}>
          ...
          <div style={{ display: 'flex', gap: 4 }}>
            {alertCount > 0 && <button onClick={dismissAll} style={{ padding: '4px 10px', borderRadius: 6, border: 'none', background: 'rgba(124,92,252,0.04)', color: '#7e7e88', fontSize: 11, cursor: 'pointer', fontFamily: T.font }}
              onMouseOver={e => { e.currentTarget.style.background = '#26262f'; e.currentTarget.style.color = '#9b9ba3' }}
              onMouseOut={e => { e.currentTarget.style.background = 'rgba(124,92,252,0.04)'; e.currentTarget.style.color = '#7e7e88' }}>Clear all</button>}
            <button onClick={onClose} style={{ ...btnBase, background: 'rgba(124,92,252,0.04)', color: '#7e7e88' }}><X size={14} /></button>
          </div>
        </div>

        {/* Content */}
        <div style={{ flex: 1, overflowY: 'auto', padding: '10px 12px' }}>
          {loading && <p style={{ textAlign: 'center', padding: 20, color: '#7e7e88', fontSize: 13, fontFamily: T.font }}>Loading...</p>}

          {!loading && totalCount === 0 && <p style={{ textAlign: 'center', padding: 40, color: '#56565e', fontSize: 13, fontFamily: T.font }}>No notifications</p>}

          {/* Section: Partnership signals */}
          {partnershipAlerts.length > 0 && (
            <div style={{ marginBottom: 16 }}>
              <div style={{ padding: '4px 8px 8px', fontSize: 11, color: '#7e7e88', fontFamily: T.font, fontWeight: 500, letterSpacing: '0.5px' }}>PARTNERSHIP SIGNALS</div>
              {partnershipAlerts.map(alert => (
                <div key={alert.id} style={{ ...pillStyle, background: 'rgba(6,214,160,0.03)', border: '1px solid rgba(6,214,160,0.08)' }}>
                  ...
                  <div style={{ flex: 1, minWidth: 0 }}>
                    <span style={{ fontSize: 13, color: '#9b9ba3', fontWeight: 400, fontFamily: T.font, display: 'block', lineHeight: 1.4 }}>{alert.title}</span>
                  </div>
          ...
          {draftActions.length > 0 && (
            <div style={{ marginBottom: 16 }}>
              <div style={{ padding: '4px 8px 8px', fontSize: 11, color: '#7e7e88', fontFamily: T.font, fontWeight: 500, letterSpacing: '0.5px' }}>SUGGESTED ACTIONS</div>
              {draftActions.map(draft => (
                <div key={draft.id} style={{ ...pillStyle, background: 'rgba(124,92,252,0.03)', border: '1px solid rgba(124,92,252,0.08)' }}>
                  ...
                  <div style={{ flex: 1, minWidth: 0 }}>
                    <span style={{ fontSize: 13, color: '#9b9ba3', fontWeight: 500, fontFamily: T.font }}>{draft.payload?.entity || 'Action'}: </span>
          ...
          {alertCount > partnershipAlerts.length && (
            <div style={{ ...pillStyle, background: 'rgba(245,158,11,0.03)', border: '1px solid rgba(245,158,11,0.06)', cursor: 'pointer' }}
              ...
              <span style={{ fontSize: 13, color: '#9b9ba3', fontFamily: T.font }}>{alertCount - partnershipAlerts.length} more alert{alertCount - partnershipAlerts.length !== 1 ? 's' : ''} — tap to brief</span>
            </div>
          )}
        </div>
      </div>
