...
export default function EmailDraft({ text, defaultSender, defaultTo }) {
  ...
  const handleRewrite = async (prompt) => {
    setRewriting(true)
    try {
      const res = await fetch('https://api.vanhawke.agency/api/rewrite-email', { method: 'POST', headers: { 'Content-Type': 'application/json' }, body: JSON.stringify({ prompt, body: currentBody, recipientEmail: currentTo, senderEmail: selectedSender?.email || 'sunny@vanhawke.agency' }) })
      const data = await res.json()
    ...
    <div style={{ margin: '12px 0', borderRadius: 14, border: '0.5px solid rgba(0,0,0,0.08)', background: 'rgba(0,0,0,0.02)' }}>
      ...
      <div style={{ padding: '14px 18px 12px', borderBottom: '0.5px solid rgba(0,0,0,0.08)', display: 'flex', justifyContent: 'space-between', alignItems: 'flex-start' }}>
        <div style={{ flex: 1 }}>
          ...
          <div style={{ fontSize: 13, color: '#6B6B6B', fontFamily: T.font, marginBottom: 4, display: 'flex', alignItems: 'center', gap: 4 }}><span style={{ color: '#A0A0A0' }}>To:</span> <input value={currentTo} onChange={e => setCurrentTo(e.target.value)} placeholder="add recipient…" style={{ border: 'none', outline: 'none', fontSize: 13, color: '#6B6B6B', fontFamily: T.font, flex: 1, background: 'transparent', padding: 0 }} /></div>
