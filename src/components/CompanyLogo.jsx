...
export default function CompanyLogo({ domain, name, size = 24, style = {} }) {
  ...
  if (!logoDomain || failed) {
    return (
      <div style={{
        ...
        background: 'rgba(212,167,106,0.08)', display: 'flex', alignItems: 'center', justifyContent: 'center',
        fontSize: size * 0.4, fontWeight: 600, color: 'rgba(212,167,106,0.5)', ...style,
      }}>{initial}</div>
    )
  }
  ...
  return (
    <img 
      ...
      style={{ width: size, height: size, borderRadius: size > 20 ? 8 : 6, flexShrink: 0, objectFit: 'contain', background: 'rgba(255,255,255,0.06)', ...style }}
    />
  )
}
