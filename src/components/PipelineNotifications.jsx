...
const T = {
  bg: '#000000', surface: 'rgba(0,0,0,0.03)',
...
const TYPE_CONFIG = {
  ...
  new_partnership: { icon: Trophy, color: '#0A0A0A', bg: 'rgba(0,0,0,0.05)', label: 'F1 Partnership' },
    ...
    <div style={{ background: T.surface, borderRadius: 18, border: `1px solid ${T.border}`, overflow: 'hidden', fontFamily: T.font }}>
      ...
      <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', padding: '12px 16px', borderBottom: `1px solid ${T.border}`, cursor: 'pointer' }}
        ...
        <div style={{ display: 'flex', alignItems: 'center', gap: 8 }}>
          ...
          {unread > 0 && (
            <span style={{ fontSize: 11, fontWeight: 500, color: '#0A0A0A', background: T.red, borderRadius: 50, padding: '1px 6px', minWidth: 18, textAlign: 'center' }}>{unread}</span>
          )}
        </div>
        <div style={{ display: 'flex', alignItems: 'center', gap: 8 }}>
          {unread > 0 && (
