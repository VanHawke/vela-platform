...
export default function DocumentCard({ doc, compact = false, onView }) {
    ...
    return (
      <div onClick={() => onView?.(doc)} style={{ display: 'flex', alignItems: 'center', gap: 10, padding: '8px 10px', borderRadius: 50, cursor: 'pointer', transition: 'background 0.15s', background: 'transparent' }}
        ...
        <div style={{ minWidth: 0, flex: 1 }}>
          <p style={{ fontSize: 13, fontWeight: 500, color: 'var(--text)', margin: 0, fontFamily: 'var(--font)', overflow: 'hidden', textOverflow: 'ellipsis', whiteSpace: 'nowrap' }}>{doc.name}</p>
  ...
  return (
    <div style={{ background: 'rgba(255,255,255,0.07)', borderRadius: 50, padding: 14, border: '1.5px solid rgba(255,255,255,0.1)', boxShadow: '0 1px 2px rgba(0,0,0,0.03)' }}>
      <div style={{ display: 'flex', alignItems: 'flex-start', gap: 10, marginBottom: 8 }}>
        ...
        <div style={{ minWidth: 0, flex: 1 }}>
          <p style={{ fontSize: 14, fontWeight: 400, color: 'var(--text)', margin: 0, fontFamily: 'var(--font)', lineHeight: 1.3 }}>{doc.name}</p>
          <div style={{ display: 'flex', gap: 6, alignItems: 'center', marginTop: 4, flexWrap: 'wrap' }}>
            <span style={{ fontSize: 10, fontWeight: 400, letterSpacing: '0.04em', textTransform: 'uppercase', color: catColor, background: `${catColor}12`, padding: '2px 6px', borderRadius: 4 }}>{doc.category || 'document'}</span>
            {doc.linked_team && <span style={{ fontSize: 10, color: 'var(--text-tertiary)', fontFamily: 'var(--font)' }}>· {doc.linked_team}</span>}
            <span style={{ fontSize: 10, color: 'var(--text-tertiary)', fontFamily: 'var(--font)' }}>· {date}</span>
          </div>
        </div>
      </div>
      {intel.summary && <p style={{ fontSize: 12, color: 'var(--text-secondary)', margin: '0 0 8px', fontFamily: 'var(--font)', lineHeight: 1.45 }}>{intel.summary}</p>}
      {(intel.key_stats?.length > 0) && (
        <div style={{ display: 'flex', flexWrap: 'wrap', gap: 4, marginBottom: 6 }}>
          {intel.key_stats.slice(0, 3).map((s, i) => (
            <span key={i} style={{ fontSize: 10, padding: '2px 6px', borderRadius: 4, background: 'rgba(255,255,255,0.04)', color: 'var(--text-secondary)', fontFamily: 'var(--font)' }}>{s}</span>
          ))}
        </div>
      )}
    </div>
