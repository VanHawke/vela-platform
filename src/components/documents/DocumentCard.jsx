...
export default function DocumentCard({ doc, compact = false, onView }) {
  ...
  return (
    <div style={{ background: 'var(--border)', borderRadius: 50, padding: 14, border: '1.5px solid var(--border)', boxShadow: '0 1px 2px rgba(0,0,0,0.03)' }}>
      ...
      {(intel.key_stats?.length > 0) && (
        <div style={{ display: 'flex', flexWrap: 'wrap', gap: 4, marginBottom: 6 }}>
          {intel.key_stats.slice(0, 3).map((s, i) => (
            <span key={i} style={{ fontSize: 10, padding: '2px 6px', borderRadius: 4, background: 'var(--border)', color: 'var(--text-secondary)', fontFamily: 'var(--font)' }}>{s}</span>
          ))}
        </div>
      )}
    </div>
