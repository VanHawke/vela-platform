  ...
  return (
    <div style={{ background: 'rgba(255,255,255,0.07)', borderRadius: 18, padding: '16px 20px', border: '0.5px solid rgba(255,255,255,0.1)', boxShadow: 'none' }}>
      <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: 10 }}>
        ...
        <button onClick={() => fileRef.current?.click()} disabled={uploading} style={{ display: 'flex', alignItems: 'center', gap: 4, fontSize: 10, fontWeight: 500, color: 'var(--accent)', background: 'rgba(255,255,255,0.07)', border: '0.5px solid rgba(255,255,255,0.1)', borderRadius: 6, padding: '4px 8px', cursor: 'pointer', fontFamily: 'var(--font)' }}>
      ...
      {loading ? (
        <div style={{ display: 'flex', flexDirection: 'column', gap: 6 }}>
          {[...Array(2)].map((_, i) => <div key={i} style={{ height: 40, background: 'rgba(255,255,255,0.07)', borderRadius: 50, animation: 'pulse 1.5s infinite' }} />)}
        </div>
      ) : documents.length === 0 ? (
