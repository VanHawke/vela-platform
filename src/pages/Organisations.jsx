...
function OrgLogo({ domain, name, size = 36 }) {
  if (domain) {
    return (
      <div style={{ width: size, height: size, borderRadius: size > 30 ? 10 : 8, background: 'rgba(25,25,25,0.40)', border: '0.5px solid rgba(167,139,250,0.50)', display: 'flex', alignItems: 'center', justifyContent: 'center', flexShrink: 0, overflow: 'hidden' }}>
...
export default function Organisations({ user }) {
  ...
  const glass = { margin: '0 16px', padding: '12px 20px', borderRadius: 18, background: 'rgba(25,25,25,0.40)', backdropFilter: 'blur(40px)', WebkitBackdropFilter: 'blur(40px)', border: '0.5px solid rgba(167,139,250,0.50)', boxShadow: '0 8px 36px rgba(0,0,0,0.3)', display: 'flex', alignItems: 'center', justifyContent: 'space-between' }
  const listCard = { background: 'rgba(25,25,25,0.40)', borderRadius: 50, padding: '14px 18px', border: '0.5px solid rgba(167,139,250,0.50)', boxShadow: 'none', display: 'flex', alignItems: 'center', justifyContent: 'space-between', transition: 'box-shadow 0.15s ease', cursor: 'pointer' }
    ...
    <div style={{ flex: 1, display: 'flex', flexDirection: 'column', overflow: 'hidden', paddingTop: 8 }}>
          ...
          {[
            ...
            return (
              <button key={chip.id} onClick={() => setOpenPopover(isOpen ? null : chip.id)} style={{
                ...
                border: isActive ? '1.5px solid rgba(167,139,250,0.25)' : '0.5px solid rgba(167,139,250,0.08)',
                background: isActive ? 'rgba(167,139,250,0.12)' : isOpen ? 'rgba(25,25,25,0.40)' : 'rgba(25,25,25,0.40)',
                ...
                {chip.label}
                {chip.count > 0 && <span style={{ fontSize: 11, fontWeight: 400, background: 'rgba(167,139,250,0.25)', borderRadius: 50, padding: '1px 5px' }}>{chip.count}</span>}
          ...
          {openPopover && (
            <div style={{ position: 'absolute', top: 36, left: 0, zIndex: 200, background: 'rgba(25,25,25,0.50)', backdropFilter: 'blur(40px) saturate(1.6)', WebkitBackdropFilter: 'blur(40px) saturate(1.6)', border: '0.5px solid rgba(167,139,250,0.50)', borderRadius: 18, padding: 14, boxShadow: 'inset 0 2px 0 rgba(167,139,250,0.10), 0 8px 32px rgba(0,0,0,0.4)', minWidth: 280, maxWidth: 360 }}
                    ...
                    {filterOptions.industries.map(ind => {
                      ...
                      return (
                        <button key={ind} onClick={() => setFilters(p => ({ ...p, industries: sel ? p.industries.filter(i => i !== ind) : [...p.industries, ind] }))} style={{
                          padding: '4px 10px', borderRadius: 50, border: '0.5px solid rgba(167,139,250,0.50)',
                          background: sel ? 'rgba(167,139,250,0.12)' : 'rgba(25,25,25,0.40)', color: sel ? 'rgba(238,232,220,0.90)' : 'var(--text-secondary)',
              ...
              <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginTop: 12, paddingTop: 10, borderTop: '1px solid rgba(25,25,25,0.40)' }}>
                ...
                <button onClick={() => setOpenPopover(null)} style={{ fontSize: 13, fontWeight: 500, padding: '6px 16px', borderRadius: 50, background: 'rgba(167,139,250,0.12)', color: 'rgba(238,232,220,0.90)', border: 'none', cursor: 'pointer', fontFamily: 'var(--font)' }}>
          ...
          <div style={{ display: 'flex', alignItems: 'center', gap: 5, flexWrap: 'wrap', paddingBottom: 4 }}>
            ...
            {filters.lastRound && (
              <span onClick={() => setFilters(p => ({ ...p, lastRound: '' }))}
                style={{ display: 'inline-flex', alignItems: 'center', gap: 4, padding: '3px 8px 3px 10px', borderRadius: 50, background: 'rgba(167,139,250,0.08)', color: 'rgba(167,139,250,0.6)', fontSize: 12, fontWeight: 500, cursor: 'pointer', fontFamily: 'var(--font)' }}>
        ...
        <div style={{ flex: 1, overflowY: 'auto', transition: 'flex 0.3s ease', minWidth: 0 }}>
          {loading ? (
            ...
            <div style={{ display: 'flex', flexDirection: 'column', gap: 3 }}>
              {/* Column headers */}
              <div style={{ display: 'flex', alignItems: 'center', gap: 14, padding: '6px 18px 8px', fontSize: 11, color: 'rgba(167,139,250,0.30)', textTransform: 'uppercase', letterSpacing: '0.06em', fontWeight: 300, fontFamily: 'var(--font)' }}>
              ...
              {paged.map(company => (
                <div key={company.id} style={{ display: 'flex', alignItems: 'center', gap: 14, padding: '10px 18px', borderRadius: 12, background: selectedOrg?.id === company.id ? 'rgba(167,139,250,0.06)' : 'rgba(25,25,25,0.55)', border: `1px solid ${selectedOrg?.id === company.id ? 'rgba(167,139,250,0.12)' : 'rgba(167,139,250,0.50)'}`, cursor: 'pointer', transition: 'all 0.15s' }}
                  onClick={() => selectOrg(company)}
                  onMouseEnter={e => { e.currentTarget.style.background = 'rgba(167,139,250,0.06)'; e.currentTarget.style.borderColor = 'rgba(167,139,250,0.10)' }}
                  onMouseLeave={e => { e.currentTarget.style.background = selectedOrg?.id === company.id ? 'rgba(167,139,250,0.06)' : 'rgba(25,25,25,0.55)'; e.currentTarget.style.borderColor = selectedOrg?.id === company.id ? 'rgba(167,139,250,0.12)' : 'rgba(167,139,250,0.50)' }}>
                  ...
                  <div style={{ width: 130, fontSize: 12, color: 'rgba(167,139,250,0.30)', fontFamily: 'var(--font)', overflow: 'hidden', textOverflow: 'ellipsis', whiteSpace: 'nowrap' }}>{company.industry || '—'}</div>
                  <div style={{ width: 120, fontSize: 12, color: 'rgba(167,139,250,0.35)', fontFamily: 'var(--font)', overflow: 'hidden', textOverflow: 'ellipsis', whiteSpace: 'nowrap' }}>{company.country || '—'}</div>
                  <div style={{ width: 100, fontSize: 12, color: company.totalFunding ? 'rgba(6,214,160,0.5)' : 'rgba(167,139,250,0.08)', fontFamily: 'var(--font)', textAlign: 'right' }}>{company.totalFunding || '—'}</div>
                  <div style={{ flex: 1, fontSize: 12, color: 'rgba(167,139,250,0.30)', fontFamily: 'var(--font)', textAlign: 'right' }}>{company.contactCount || 0}</div>
                  <div style={{ width: 60, display: 'flex', alignItems: 'center', gap: 4, justifyContent: 'flex-end' }}>
                    ...
                    <button onClick={(e) => { e.stopPropagation(); remove(company.id) }} style={{ color: 'rgba(167,139,250,0.10)', background: 'none', border: 'none', cursor: 'pointer', padding: 2, transition: 'color 0.15s' }} onMouseEnter={e => e.currentTarget.style.color = '#ef4444'} onMouseLeave={e => e.currentTarget.style.color = 'rgba(167,139,250,0.10)'}><X style={{ width: 12, height: 12 }} /></button>
                  </div>
                </div>
              ))}
            </div>
        ...
        <div style={{ width: panelOpen ? 400 : 0, minWidth: panelOpen ? 400 : 0, transition: 'width 0.3s ease, min-width 0.3s ease, opacity 0.2s ease', opacity: panelOpen ? 1 : 0, overflow: 'hidden', marginLeft: panelOpen ? 16 : 0 }}>
          {selectedOrg && (
            <div style={{ width: 400, height: '100%', overflowY: 'auto', display: 'flex', flexDirection: 'column', gap: 14 }}>
              {/* Header card */}
              <div style={{ background: 'rgba(25,25,25,0.40)', borderRadius: 18, padding: '20px 20px 16px', border: '0.5px solid rgba(167,139,250,0.50)', boxShadow: 'none' }}>
                ...
                <div style={{ display: 'flex', gap: 8, flexWrap: 'wrap' }}>
                  {orgLinkedin ? (
                    ...
                    <span style={{ display: 'flex', alignItems: 'center', gap: 6, fontSize: 13, color: 'var(--text-tertiary)', background: 'rgba(0,0,0,0.02)', padding: '6px 12px', borderRadius: 50, fontFamily: 'var(--font)', border: '0.5px solid rgba(167,139,250,0.50)' }}>
                      <Linkedin style={{ width: 13, height: 13 }} /> No LinkedIn
                    </span>
                  )}
                  {selectedOrg.website ? (
                    <a href={selectedOrg.website.startsWith('http') ? selectedOrg.website : `https://${selectedOrg.website}`} target="_blank" rel="noopener noreferrer" style={{ display: 'flex', alignItems: 'center', gap: 6, fontSize: 13, color: 'var(--text-secondary)', background: 'rgba(25,25,25,0.40)', padding: '6px 12px', borderRadius: 50, textDecoration: 'none', fontFamily: 'var(--font)', border: '0.5px solid rgba(167,139,250,0.50)' }}>
                      <ExternalLink style={{ width: 13, height: 13 }} /> Website
                    </a>
                  ) : orgDomain ? (
                    <a href={`https://${orgDomain}`} target="_blank" rel="noopener noreferrer" style={{ display: 'flex', alignItems: 'center', gap: 6, fontSize: 13, color: 'var(--text-secondary)', background: 'rgba(25,25,25,0.40)', padding: '6px 12px', borderRadius: 50, textDecoration: 'none', fontFamily: 'var(--font)', border: '0.5px solid rgba(167,139,250,0.50)' }}>
                  ...
                  <button onClick={() => edit(selectedOrg)} style={{ display: 'flex', alignItems: 'center', gap: 6, fontSize: 13, color: 'var(--text-secondary)', background: 'rgba(25,25,25,0.40)', padding: '6px 12px', borderRadius: 50, border: '0.5px solid rgba(167,139,250,0.50)', cursor: 'pointer', fontFamily: 'var(--font)' }}>
                    Edit
                  </button>
                </div>
              </div>
              ...
              {(selectedOrg.lastRound || selectedOrg.totalFunding || selectedOrg.employees || selectedOrg.founded) && (
                <div style={{ background: 'rgba(25,25,25,0.40)', borderRadius: 18, padding: '16px 20px', border: '0.5px solid rgba(167,139,250,0.50)', boxShadow: 'none' }}>
                ...
                {loadingCompetitors ? (
                  ...
                  <div style={{ display: 'flex', flexDirection: 'column', gap: 6 }}>
                    {orgCompetitors.slice(0, 5).map((c, i) => {
                      ...
                      return (
                        <div key={i} style={{ display: 'flex', alignItems: 'flex-start', gap: 10, padding: '9px 11px', borderRadius: 9, border: '0.5px solid rgba(167,139,250,0.50)', background: 'rgba(0,0,0,0.01)', cursor: 'default', transition: 'border-color 0.12s' }}
                          onMouseOver={e => e.currentTarget.style.borderColor = 'rgba(167,139,250,0.10)'}
                          onMouseOut={e => e.currentTarget.style.borderColor = 'rgba(25,25,25,0.40)'}>
                          {/* Logo */}
                          <div style={{ width: 28, height: 28, borderRadius: 7, background: 'rgba(25,25,25,0.40)', border: '0.5px solid rgba(167,139,250,0.50)', display: 'flex', alignItems: 'center', justifyContent: 'center', flexShrink: 0, overflow: 'hidden' }}>
                            {compDomain ? (
              ...
              <div style={{ background: 'rgba(25,25,25,0.40)', borderRadius: 18, padding: '16px 20px', border: '0.5px solid rgba(167,139,250,0.50)', boxShadow: 'none' }}>
                ...
                {loadingSignals ? (
                  ...
                  <div style={{ display: 'flex', flexDirection: 'column', gap: 8 }}>
                    {orgSignals.map((s, i) => {
                      const typeColor = { funding: '#10b981', partnership: '#3b82f6', product: '#A78BFA', leadership: '#f59e0b', acquisition: '#ef4444', expansion: '#06b6d4' }[s.type] || 'var(--text-tertiary)'
                      return (
      ...
      {showForm && (
        <div onClick={e => e.target === e.currentTarget && reset()} style={{ position: 'fixed', inset: 0, background: 'rgba(0,0,0,0.3)', backdropFilter: 'blur(8px)', zIndex: 50, display: 'flex', alignItems: 'center', justifyContent: 'center', padding: 16 }}>
          <div style={{ background: 'rgba(25,25,25,0.40)', borderRadius: 50, border: '0.5px solid rgba(167,139,250,0.50)', boxShadow: '0 24px 80px rgba(167,139,250,0.10), 0 8px 24px rgba(25,25,25,0.40)', width: '100%', maxWidth: 420, padding: 24 }}>
