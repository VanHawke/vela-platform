...
function statusColor(c) {
  ...
  return { bg: 'rgba(238,238,238,0.04)', color: T.textTertiary, border: 'rgba(238,238,238,0.06)' }
}
...
function stepIcon(type) {
  ...
  if (t.includes('delay') || t.includes('wait')) return <Clock size={14} style={{ color: 'rgba(238,238,238,0.3)' }} />
  return <Send size={14} style={{ color: 'rgba(238,238,238,0.4)' }} />
}
...
function activityIcon(type) {
  ...
  if (t.includes('sent')) return <Mail size={12} style={{ color: 'rgba(238,238,238,0.4)' }} />
  return <Mail size={12} style={{ color: 'rgba(238,238,238,0.25)' }} />
}
...
export default function Lemlist({ user }) {
  ...
  // Shared styles
  const card = { background: 'rgba(238,238,238,0.02)', border: '1px solid rgba(238,238,238,0.04)', borderRadius: 12, transition: 'all 0.15s' }
  const statCard = { ...card, padding: '12px 16px', display: 'flex', flexDirection: 'column', gap: 2 }
  const sectionTitle = { fontSize: 12, fontWeight: 500, color: 'rgba(238,238,238,0.4)', textTransform: 'uppercase', letterSpacing: '0.06em', margin: '0 0 10px', fontFamily: T.font }
  ...
  return (
    <div style={{ display: 'flex', height: 'calc(100vh - 56px)', fontFamily: T.font }}>
      ...
      <div style={{ width: 280, borderRight: '1px solid rgba(238,238,238,0.04)', display: 'flex', flexDirection: 'column', flexShrink: 0 }}>
        <div style={{ padding: '16px 16px 10px', display: 'flex', alignItems: 'center', justifyContent: 'space-between' }}>
          ...
          <button onClick={loadCampaigns} style={{ width: 28, height: 28, borderRadius: 50, border: 'none', background: 'rgba(238,238,238,0.04)', cursor: 'pointer', display: 'flex', alignItems: 'center', justifyContent: 'center', color: T.textTertiary }}>
            <RefreshCw size={12} />
          </button>
        </div>
          ...
          {(() => {
            ...
            return campaigns.map(c => {
              ...
              return (
                <div key={c._id}>
                  {showHeader && (
                    <div style={{ fontSize: 10, fontWeight: 600, color: 'rgba(238,238,238,0.25)', textTransform: 'uppercase', letterSpacing: '0.1em', padding: '12px 12px 6px', marginTop: grp > 0 ? 8 : 0 }}>
                  ...
                  <div onClick={() => selectCampaign(c)} style={{
                    ...
                    border: `1px solid ${isActive ? 'rgba(255,224,194,0.15)' : 'rgba(238,238,238,0.03)'}`,
                  }}
                    onMouseEnter={e => { if (!isActive) e.currentTarget.style.background = 'rgba(238,238,238,0.04)' }}
                  ...
                  >
                    ...
                    <div style={{ display: 'flex', gap: 6, alignItems: 'center' }}>
                      ...
                      <span style={{ fontSize: 10, color: 'rgba(238,238,238,0.15)', marginLeft: 'auto' }}>{timeAgo(c.createdAt)}</span>
                    </div>
                  </div>
                </div>
              )
      ...
      <div style={{ flex: 1, display: 'flex', flexDirection: 'column', overflow: 'hidden', minWidth: 0 }}>
        {!selectedCampaign ? (
          ...
          <>
            ...
            <div style={{ padding: '16px 20px 12px', borderBottom: '1px solid rgba(238,238,238,0.04)' }}>
                ...
                <div style={{ display: 'grid', gridTemplateColumns: 'repeat(6, 1fr)', gap: 8 }}>
                  {[
                    ...
                    <div key={s.label} style={statCard}>
                      <span style={{ fontSize: 10, color: 'rgba(238,238,238,0.3)', textTransform: 'uppercase', letterSpacing: '0.05em' }}>{s.label}</span>
                      <span style={{ fontSize: 18, fontWeight: 600, color: s.danger && s.value > 0 ? 'rgba(255,59,48,0.8)' : s.accent && s.value > 0 ? 'rgba(6,214,160,0.8)' : 'rgba(238,238,238,0.7)' }}>{s.value}{s.rate !== undefined ? <span style={{ fontSize: 11, fontWeight: 400, color: T.textTertiary, marginLeft: 4 }}>{s.rate}%</span> : null}</span>
                    </div>
                  ))}
                </div>
              )}
              ...
              {sequenceSteps.length > 0 && (
                <div style={{ marginBottom: 24 }}>
                  ...
                  <div style={{ display: 'flex', flexDirection: 'column', gap: 0, position: 'relative', paddingLeft: 20 }}>
                    {/* Vertical line */}
                    <div style={{ position: 'absolute', left: 6, top: 8, bottom: 8, width: 1, background: 'rgba(238,238,238,0.06)' }} />
                      ...
                      <div key={step._id || idx} style={{ display: 'flex', gap: 12, alignItems: 'flex-start', padding: '8px 0', position: 'relative' }}>
                        ...
                        <div style={{ flex: 1, ...card, padding: '10px 14px' }}>
                          <div style={{ display: 'flex', alignItems: 'center', gap: 8, marginBottom: 2 }}>
                            {stepIcon(step.type)}
                            <span style={{ fontSize: 12, fontWeight: 500, color: 'rgba(238,238,238,0.6)', textTransform: 'capitalize' }}>{step.type === 'linkedin' ? 'LinkedIn Message' : step.type === 'email' ? 'Email' : 'Action'}</span>
                            <span style={{ fontSize: 10, color: T.textTertiary, marginLeft: 'auto' }}>Step {step.stepNumber} · {step.count} sent</span>
                          </div>
                          {step.subject && <div style={{ fontSize: 13, color: 'rgba(238,238,238,0.5)', marginTop: 4 }}>Subject: {step.subject}</div>}
                        </div>
                      </div>
                    ))}
                  </div>
                  ...
                  {leads.map((lead, idx) => {
                    ...
                    return (
                      <div key={lead._id || idx} onClick={() => selectLead(lead)} style={{
                        ...
                        borderColor: isSelected ? 'rgba(255,224,194,0.15)' : 'rgba(238,238,238,0.04)',
                      }}
                        onMouseEnter={e => { if (!isSelected) e.currentTarget.style.background = 'rgba(238,238,238,0.04)' }}
                        onMouseLeave={e => { if (!isSelected) e.currentTarget.style.background = 'rgba(238,238,238,0.02)' }}
                      >
                        <CompanyLogo name={lead.companyName || lead.firstName} size={30} />
                        <div style={{ flex: 1, minWidth: 0 }}>
                          <div style={{ fontSize: 13, fontWeight: 500, color: 'rgba(238,238,238,0.7)', whiteSpace: 'nowrap', overflow: 'hidden', textOverflow: 'ellipsis' }}>
                        ...
                        <div style={{ display: 'flex', flexDirection: 'column', alignItems: 'flex-end', gap: 2, flexShrink: 0 }}>
                          <span style={{ fontSize: 10, padding: '1px 6px', borderRadius: 5, fontWeight: 500, textTransform: 'capitalize',
                            background: isReply ? 'rgba(6,214,160,0.08)' : isBounce ? 'rgba(255,59,48,0.08)' : 'rgba(238,238,238,0.04)',
                            color: isReply ? 'rgba(6,214,160,0.7)' : isBounce ? 'rgba(255,59,48,0.7)' : T.textTertiary,
                          }}>{lastType || 'active'}</span>
                          <span style={{ fontSize: 10, color: 'rgba(238,238,238,0.2)' }}>{lead.activities} events · {timeAgo(lead.lastActivity)}</span>
                        </div>
                        <ChevronRight size={14} style={{ color: 'rgba(238,238,238,0.15)', flexShrink: 0 }} />
                      </div>
                    )
                  })}
                </div>
      ...
      {selectedLead && (
        <div style={{ width: 380, borderLeft: '1px solid rgba(238,238,238,0.04)', display: 'flex', flexDirection: 'column', flexShrink: 0, background: 'rgba(238,238,238,0.01)' }}>
          {/* Header */}
          <div style={{ padding: '14px 16px', borderBottom: '1px solid rgba(238,238,238,0.04)', display: 'flex', alignItems: 'center', gap: 10 }}>
            <div style={{ flex: 1, minWidth: 0 }}>
            ...
            <button onClick={() => setSelectedLead(null)} style={{ width: 28, height: 28, borderRadius: 8, background: 'rgba(238,238,238,0.04)', border: 'none', cursor: 'pointer', display: 'flex', alignItems: 'center', justifyContent: 'center', color: 'rgba(238,238,238,0.3)' }}>
              <X size={14} />
            </button>
          </div>
          ...
          <div style={{ flex: 1, overflowY: 'auto', padding: '12px 16px' }}>
            ...
            {leadActivities.length === 0 ? (
              ...
              <div style={{ display: 'flex', flexDirection: 'column', gap: 0, position: 'relative', paddingLeft: 20 }}>
                <div style={{ position: 'absolute', left: 5, top: 6, bottom: 6, width: 1, background: 'rgba(238,238,238,0.04)' }} />
                {leadActivities.map((a, i) => {
                  ...
                  return (
                    <div key={a._id || i} style={{ display: 'flex', gap: 10, padding: '8px 0', position: 'relative' }}>
                      ...
                      <div style={{ flex: 1, ...card, padding: '10px 12px', background: isReply ? 'rgba(6,214,160,0.03)' : card.background, borderColor: isReply ? 'rgba(6,214,160,0.1)' : 'rgba(238,238,238,0.04)' }}>
                        <div style={{ display: 'flex', alignItems: 'center', gap: 6, marginBottom: 4 }}>
                          {activityIcon(a.type)}
                          <span style={{ fontSize: 12, fontWeight: 500, color: 'rgba(238,238,238,0.5)', textTransform: 'capitalize' }}>{typeLabel}</span>
                          <span style={{ fontSize: 10, color: 'rgba(238,238,238,0.15)', marginLeft: 'auto' }}>{a.createdAt ? new Date(a.createdAt).toLocaleString('en-GB', { day: 'numeric', month: 'short', hour: '2-digit', minute: '2-digit' }) : ''}</span>
                        </div>
                        ...
                        {/* Campaign name */}
                        {a.campaignName && <div style={{ fontSize: 11, color: 'rgba(238,238,238,0.2)' }}>{a.campaignName}</div>}
                        {/* Message content if sent/replied */}
                        {(isSent || isReply) && a.text && (
                          <div style={{ marginTop: 6, padding: '8px 10px', borderRadius: 8, background: 'rgba(238,238,238,0.02)', border: '1px solid rgba(238,238,238,0.03)', fontSize: 12, color: 'rgba(238,238,238,0.4)', lineHeight: 1.55, maxHeight: 120, overflowY: 'auto', whiteSpace: 'pre-wrap' }}>
                            {a.text}
                          </div>
                        )}
                        {(isSent || isReply) && a.html && !a.text && (
                          <div style={{ marginTop: 6, padding: '8px 10px', borderRadius: 8, background: 'rgba(238,238,238,0.02)', border: '1px solid rgba(238,238,238,0.03)', fontSize: 12, color: 'rgba(238,238,238,0.4)', lineHeight: 1.55, maxHeight: 120, overflowY: 'auto' }}
