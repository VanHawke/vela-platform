...
function statusColor(c) {
  ...
  return { bg: 'rgba(255,255,255,0.04)', color: T.textTertiary, border: 'rgba(255,255,255,0.06)' }
}
...
function stepIcon(type) {
  ...
  return <Send size={14} style={{ color: 'rgba(255,255,255,0.4)' }} />
}
...
function activityIcon(type) {
  ...
  if (t.includes('sent')) return <Mail size={12} style={{ color: 'rgba(255,255,255,0.4)' }} />
  return <Mail size={12} style={{ color: 'rgba(255,255,255,0.25)' }} />
}
...
export default function Lemlist({ user }) {
  ...
  // Shared styles
  const card = { background: 'rgba(255,255,255,0.02)', border: '1px solid rgba(255,255,255,0.04)', borderRadius: 12, transition: 'all 0.15s' }
  const statCard = { ...card, padding: '12px 16px', display: 'flex', flexDirection: 'column', gap: 2 }
  const sectionTitle = { fontSize: 12, fontWeight: 500, color: 'rgba(255,255,255,0.4)', textTransform: 'uppercase', letterSpacing: '0.06em', margin: '0 0 10px', fontFamily: T.font }
  ...
  return (
    <div style={{ display: 'flex', height: 'calc(100vh - 56px)', fontFamily: T.font }}>
      ...
      <div style={{ width: 280, borderRight: '1px solid rgba(255,255,255,0.04)', display: 'flex', flexDirection: 'column', flexShrink: 0 }}>
          ...
          {(() => {
            ...
            return campaigns.map(c => {
              ...
              return (
                <div key={c._id}>
                  {showHeader && (
                    <div style={{ fontSize: 10, fontWeight: 600, color: 'rgba(255,255,255,0.25)', textTransform: 'uppercase', letterSpacing: '0.1em', padding: '12px 12px 6px', marginTop: grp > 0 ? 8 : 0 }}>
                  ...
                  >
                    <div style={{ fontSize: 13, fontWeight: 500, color: isActive ? '#fff' : 'rgba(255,255,255,0.65)', marginBottom: 4, lineHeight: 1.35 }}>{c.name}</div>
                    <div style={{ display: 'flex', gap: 6, alignItems: 'center' }}>
                      ...
                      <span style={{ fontSize: 10, color: 'rgba(255,255,255,0.15)', marginLeft: 'auto' }}>{timeAgo(c.createdAt)}</span>
                    </div>
                  </div>
                </div>
              )
                ...
                <div style={{ display: 'grid', gridTemplateColumns: 'repeat(6, 1fr)', gap: 8 }}>
                  {[
                    ...
                    <div key={s.label} style={statCard}>
                      <span style={{ fontSize: 10, color: 'rgba(255,255,255,0.3)', textTransform: 'uppercase', letterSpacing: '0.05em' }}>{s.label}</span>
                      <span style={{ fontSize: 18, fontWeight: 600, color: s.danger && s.value > 0 ? 'rgba(255,59,48,0.8)' : s.accent && s.value > 0 ? 'rgba(6,214,160,0.8)' : 'rgba(255,255,255,0.7)' }}>{s.value}{s.rate !== undefined ? <span style={{ fontSize: 11, fontWeight: 400, color: T.textTertiary, marginLeft: 4 }}>{s.rate}%</span> : null}</span>
                    </div>
                  ))}
                </div>
              )}
      ...
      {selectedLead && (
        <div style={{ width: 380, borderLeft: '1px solid rgba(255,255,255,0.04)', display: 'flex', flexDirection: 'column', flexShrink: 0, background: 'rgba(255,255,255,0.01)' }}>
          {/* Header */}
          <div style={{ padding: '14px 16px', borderBottom: '1px solid rgba(255,255,255,0.04)', display: 'flex', alignItems: 'center', gap: 10 }}>
            ...
            <button onClick={() => setSelectedLead(null)} style={{ width: 28, height: 28, borderRadius: 8, background: 'rgba(255,255,255,0.04)', border: 'none', cursor: 'pointer', display: 'flex', alignItems: 'center', justifyContent: 'center', color: 'rgba(255,255,255,0.3)' }}>
              <X size={14} />
            </button>
          </div>
                    ...
                    <div key={a._id || i} style={{ display: 'flex', gap: 10, padding: '8px 0', position: 'relative' }}>
                      ...
                      <div style={{ flex: 1, ...card, padding: '10px 12px', background: isReply ? 'rgba(6,214,160,0.03)' : card.background, borderColor: isReply ? 'rgba(6,214,160,0.1)' : 'rgba(255,255,255,0.04)' }}>
                        <div style={{ display: 'flex', alignItems: 'center', gap: 6, marginBottom: 4 }}>
                          {activityIcon(a.type)}
                          <span style={{ fontSize: 12, fontWeight: 500, color: 'rgba(255,255,255,0.5)', textTransform: 'capitalize' }}>{typeLabel}</span>
                          <span style={{ fontSize: 10, color: 'rgba(255,255,255,0.15)', marginLeft: 'auto' }}>{a.createdAt ? new Date(a.createdAt).toLocaleString('en-GB', { day: 'numeric', month: 'short', hour: '2-digit', minute: '2-digit' }) : ''}</span>
                        </div>
                        {/* Subject line if available */}
                        {a.subject && <div style={{ fontSize: 12, color: 'rgba(255,255,255,0.45)', marginBottom: 2 }}>Subject: {a.subject}</div>}
                        {/* Campaign name */}
                        {a.campaignName && <div style={{ fontSize: 11, color: 'rgba(255,255,255,0.2)' }}>{a.campaignName}</div>}
                        {/* Message content if sent/replied */}
                        {(isSent || isReply) && a.text && (
                          <div style={{ marginTop: 6, padding: '8px 10px', borderRadius: 8, background: 'rgba(255,255,255,0.02)', border: '1px solid rgba(255,255,255,0.03)', fontSize: 12, color: 'rgba(255,255,255,0.4)', lineHeight: 1.55, maxHeight: 120, overflowY: 'auto', whiteSpace: 'pre-wrap' }}>
                            {a.text}
                          </div>
                        )}
                        {(isSent || isReply) && a.html && !a.text && (
                          <div style={{ marginTop: 6, padding: '8px 10px', borderRadius: 8, background: 'rgba(255,255,255,0.02)', border: '1px solid rgba(255,255,255,0.03)', fontSize: 12, color: 'rgba(255,255,255,0.4)', lineHeight: 1.55, maxHeight: 120, overflowY: 'auto' }}
