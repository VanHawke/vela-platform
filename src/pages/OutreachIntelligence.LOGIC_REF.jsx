...
Be direct. Use web search for current company intelligence if needed.`
  ...
  const urgencyColor = (u) => u >= 3 ? 'rgba(255,59,48,0.6)' : u >= 2 ? 'rgba(245,158,11,0.5)' : u >= 1 ? 'rgba(124,92,252,0.4)' : 'rgba(0,0,0,0.06)'
  const card = { background: 'rgba(25,25,25,0.50)', backdropFilter: 'blur(24px) saturate(1.2)', WebkitBackdropFilter: 'blur(24px) saturate(1.2)', border: '0.5px solid rgba(0,0,0,0.08)', borderRadius: 14, padding: 16, boxShadow: '0 8px 32px rgba(0,0,0,0.3), 0 1px 0 rgba(0,0,0,0.08) inset' }
      ...
      <div style={{ flex: 1, display: 'flex', flexDirection: 'column', overflow: 'hidden', minWidth: 0 }}>
        ...
        <div style={{ padding: '16px 20px 0' }}>
          <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', marginBottom: 12 }}>
            ...
            <button onClick={loadData} style={{ padding: '6px 12px', borderRadius: 8, background: 'rgba(25,25,25,0.40)', border: '0.5px solid rgba(0,0,0,0.08)', color: T.textTertiary, fontSize: 12, cursor: 'pointer', fontFamily: T.font, display: 'flex', alignItems: 'center', gap: 4 }}><RefreshCw size={10} /> Refresh</button>
          </div>
                ...
                <div style={{ display: 'flex', gap: 4, marginBottom: 2 }}>
                  {['F1', 'Formula E', 'MotoGP', 'WEC'].map(s => (
                    <button key={s} onClick={e => { e.stopPropagation(); setRaceSeries(s); setNextRace(allNextRaces[s] || null) }}
                      style={{ padding: '2px 8px', borderRadius: 50, border: raceSeries === s ? '1px solid rgba(0,212,170,0.3)' : '1px solid rgba(0,0,0,0.04)', background: raceSeries === s ? 'rgba(0,212,170,0.08)' : 'transparent', color: raceSeries === s ? 'rgba(0,212,170,0.7)' : 'rgba(0,0,0,0.10)', fontSize: 9, cursor: 'pointer', fontFamily: T.font, fontWeight: 400, transition: 'all 0.15s' }}>{s}</button>
                  ))}
                </div>
                <div style={{ display: 'flex', alignItems: 'center', gap: 10 }}>
                  <Calendar size={14} style={{ color: daysToRace <= 14 ? 'rgba(0,212,170,0.5)' : 'rgba(0,0,0,0.10)', flexShrink: 0 }} />
                  <div>
        ...
        <div style={{ flex: 1, overflowY: 'auto', padding: '0 20px 20px' }}>
            ...
            return (
              <>
                ...
                {priorityItems.length === 0 ? (
                  <div style={{ padding: '14px 16px', borderRadius: 10, marginBottom: 16, background: 'rgba(0,0,0,0.02)', border: '0.5px dashed rgba(0,0,0,0.10)', fontSize: 12, color: T.textTertiary, fontWeight: 300, lineHeight: 1.5 }}>
                  ...
                  priorityItems.map(item => (
                    <div key={`${item.kind}-${item.id}`} style={{ display: 'flex', gap: 10, alignItems: 'flex-start', padding: '10px 12px', borderRadius: 10, marginBottom: 4, background: item.bg, border: `0.5px solid ${item.border}`, cursor: 'pointer', transition: 'all 0.15s' }}
                      ...
                      <div style={{ flex: 1, minWidth: 0 }}>
                        ...
                        <div style={{ fontSize: 14, fontWeight: 400, color: '#0A0A0A', overflow: 'hidden', textOverflow: 'ellipsis', whiteSpace: 'nowrap' }}>{item.title}</div>
                ...
                <div style={{ height: 20, borderBottom: '0.5px solid rgba(0,0,0,0.05)', marginBottom: 16 }} />
              </>
            )
          })()}
          ...
          {tasks.length > 0 && (<>
            ...
            {tasks.slice(0, 8).map(task => {
              ...
              return (
                <div key={task.id} style={{ display: 'flex', gap: 10, alignItems: 'flex-start', padding: '9px 12px', borderRadius: 10, marginBottom: 4, background: 'rgba(0,0,0,0.02)', border: '0.5px solid rgba(0,0,0,0.08)', cursor: 'pointer', transition: 'all 0.15s' }}
                  ...
                  <button onClick={(e) => { e.stopPropagation(); toggleTask(task) }} style={{ background: 'none', border: 'none', cursor: 'pointer', padding: 0, marginTop: 2, color: '#6B6B6B' }}>
                    <Square size={14} />
                  </button>
                  <div style={{ flex: 1, minWidth: 0 }}>
                    <div style={{ display: 'flex', alignItems: 'center', gap: 6, marginBottom: 2 }}>
                    ...
                    <div style={{ fontSize: 14, fontWeight: 400, color: '#6B6B6B', overflow: 'hidden', textOverflow: 'ellipsis', whiteSpace: 'nowrap' }}>
                  ...
                  <ChevronRight size={12} style={{ color: '#A0A0A0', flexShrink: 0, marginTop: 8 }} />
                </div>
              )
            })}
            <div style={{ height: 16 }} />
          ...
          {priorityActions.slice(0, 30).map((action, i) => {
            ...
            return (
              <div key={action.id} onClick={() => getKikoRec(action)} style={{
                ...
                background: isSelected ? 'rgba(0,0,0,0.02)' : 'rgba(124,92,252,0.015)',
                border: `1px solid ${isSelected ? 'rgba(124,92,252,0.2)' : 'rgba(25,25,25,0.35)'}`,
              }}
                onMouseEnter={e => { if (!isSelected) { e.currentTarget.style.background = 'rgba(25,25,25,0.35)'; e.currentTarget.style.borderColor = 'rgba(0,0,0,0.04)' }}}
              ...
              >
                ...
                <div style={{ flex: 1, minWidth: 0 }}>
                  ...
                  <div style={{ fontSize: 14, fontWeight: 400, color: '#6B6B6B', marginBottom: 3, overflow: 'hidden', textOverflow: 'ellipsis', whiteSpace: 'nowrap' }}>{d.company || 'Unknown'}{d.contact ? ` — ${d.contact}` : ''}</div>
                  <div style={{ display: 'flex', gap: 10, alignItems: 'center' }}>
                    ...
                    <span style={{ fontSize: 11, color: '#6B6B6B', fontWeight: 300 }}>{action.prob}%</span>
                  </div>
                </div>
                <ChevronRight size={12} style={{ color: '#A0A0A0', flexShrink: 0, marginTop: 8 }} />
              </div>
      ...
      <div style={{ flex: 1, borderLeft: '1px solid rgba(25,25,25,0.40)', display: 'flex', flexDirection: 'column', background: 'rgba(124,92,252,0.01)', flexShrink: 0, minWidth: 0 }}>
        ...
        <div style={{ flex: 1, overflowY: 'auto', padding: 16 }}>
          {!selectedAction && !kikoLoading && (
            <div style={{ textAlign: 'center', padding: '60px 20px' }}>
              <Target size={20} style={{ color: '#A0A0A0', margin: '0 auto 10px', display: 'block' }} />
          ...
          {kikoRec && selectedAction && (
            <div>
              ...
              <div style={{ marginBottom: 12, padding: '10px 12px', borderRadius: 8, background: 'rgba(25,25,25,0.30)', border: '1px solid rgba(25,25,25,0.40)' }}>
                ...
                <div style={{ fontSize: 14, color: '#6B6B6B', fontWeight: 400 }}>
              ...
              {/* Kiko's analysis */}
              <div style={{ fontSize: 14, color: '#6B6B6B', fontWeight: 300, lineHeight: 1.65 }}>
        ...
        {selectedAction && kikoRec && !kikoLoading && (
          <div style={{ padding: '10px 16px', borderTop: '1px solid rgba(25,25,25,0.40)', display: 'flex', gap: 6, flexWrap: 'wrap' }}>
            <button onClick={() => getKikoRec(selectedAction)} style={{ padding: '6px 12px', borderRadius: 8, fontSize: 12, fontWeight: 400, border: '1px solid rgba(0,0,0,0.08)', background: 'rgba(0,0,0,0.03)', color: 'rgba(124,92,252,0.7)', cursor: 'pointer', fontFamily: T.font, display: 'flex', alignItems: 'center', gap: 4 }}><RefreshCw size={10} /> Regenerate</button>
            <button onClick={() => navigator.clipboard.writeText(kikoRec)} style={{ padding: '6px 12px', borderRadius: 8, fontSize: 12, fontWeight: 400, border: '0.5px solid rgba(0,0,0,0.08)', background: 'transparent', color: T.textTertiary, cursor: 'pointer', fontFamily: T.font }}>Copy</button>
          </div>
        )}
      </div>
    </div>
