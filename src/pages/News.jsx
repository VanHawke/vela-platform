...
function ArticleCard({ article, onStar, featured = false }) {
  ...
  if (featured) {
    ...
    return (
      <div style={{ background: T.surface, border: `1.5px solid ${T.border}`, borderLeft: `3px solid ${sig.text || 'var(--accent)'}`, borderRadius: 16, padding: '20px 24px', cursor: 'default', transition: 'border-color 0.15s' }}
        ...
        <div style={{ display: 'flex', alignItems: 'center', gap: 8, marginBottom: 12 }}>
          ...
          <button onClick={() => onStar(article)} style={{ background: 'none', border: 'none', cursor: 'pointer', padding: 2, color: article.is_starred ? T.yellow : 'var(--accent)' }}><Star size={13} fill={article.is_starred ? T.yellow : 'none'} /></button>
        </div>
        ...
        <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between' }}>
          <span style={{ fontSize: 11, color: 'var(--ring)' }}>{article.source_name}</span>
  ...
  return (
    <div style={{ display: 'flex', alignItems: 'center', gap: 14, padding: '10px 18px', borderRadius: 12, background: T.surface, border: '0.5px solid var(--ring)', cursor: 'default', transition: 'all 0.15s' }}
      onMouseEnter={e => { e.currentTarget.style.background = 'var(--accent)'; e.currentTarget.style.borderColor = T.border }}
      onMouseLeave={e => { e.currentTarget.style.background = T.surface; e.currentTarget.style.borderColor = 'var(--ring)' }}>
      <div style={{ flex: 1, minWidth: 0 }}>
        ...
        <div style={{ fontSize: 11, color: 'var(--ring)', marginTop: 2 }}>{article.source_name} · {timeAgo(article.published_at)}</div>
      </div>
      <button onClick={() => onStar(article)} style={{ background: 'none', border: 'none', cursor: 'pointer', padding: 2, color: article.is_starred ? T.yellow : 'var(--accent)', flexShrink: 0 }}><Star size={12} fill={article.is_starred ? T.yellow : 'none'} /></button>
      {article.url && <a href={article.url} target="_blank" rel="noopener noreferrer" style={{ fontSize: 11, color: T.textTertiary, textDecoration: 'none', display: 'flex', alignItems: 'center', gap: 3, flexShrink: 0, opacity: 0.5, transition: 'opacity 0.15s', padding: '4px 10px', borderRadius: 50, border: '0.5px solid var(--ring)' }} onMouseEnter={e => e.currentTarget.style.opacity = '1'} onMouseLeave={e => e.currentTarget.style.opacity = '0.5'}>Read <ExternalLink size={9} /></a>}
    </div>
  )
}
