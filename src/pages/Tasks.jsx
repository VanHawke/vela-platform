...
export default function Tasks({ user }) {
  ...
  const priorityColor = (task) => {
    ...
    return 'rgba(255,224,194,0.3)'
  }

  const card = { background: 'rgba(238,238,238,0.02)', border: `1px solid rgba(238,238,238,0.04)`, borderRadius: 12, padding: '12px 14px', marginBottom: 6, cursor: 'pointer', display: 'flex', gap: 10, alignItems: 'flex-start', transition: 'all 0.15s' }
  const inp = { width: '100%', background: 'rgba(238,238,238,0.03)', border: `0.5px solid rgba(238,238,238,0.06)`, borderRadius: 8, padding: '8px 12px', fontSize: 14, color: T.text, fontFamily: T.font, fontWeight: 300, outline: 'none' }
    ...
    <div style={{ display: 'flex', height: 'calc(100vh - 56px)', fontFamily: T.font }}>
      ...
      <div style={{ flex: 1, display: 'flex', flexDirection: 'column', overflow: 'hidden', minWidth: 0 }}>
        <div style={{ padding: '16px 20px 10px', display: 'flex', alignItems: 'center', justifyContent: 'space-between' }}>
          ...
          <button onClick={() => setShowForm(!showForm)} style={{ padding: '6px 14px', borderRadius: 8, background: 'rgba(255,224,194,0.08)', border: '1px solid rgba(255,224,194,0.15)', color: 'rgba(255,224,194,0.7)', fontSize: 12, cursor: 'pointer', fontFamily: T.font, display: 'flex', alignItems: 'center', gap: 4 }}><Plus size={12} /> New</button>
        </div>
        ...
        <div style={{ display: 'flex', gap: 6, padding: '0 20px 10px', flexWrap: 'wrap' }}>
          {[['todo', `Outstanding (${todoCount})`], ['overdue', `Overdue (${overdueCount})`], ['done', 'Completed'], ['all', 'All']].map(([key, label]) => (
            <button key={key} onClick={() => setFilter(key)} style={{
              ...
              border: `1px solid ${filter === key ? 'rgba(255,224,194,0.2)' : 'rgba(238,238,238,0.06)'}`,
              background: filter === key ? 'rgba(255,224,194,0.08)' : 'transparent',
              color: filter === key ? 'rgba(255,224,194,0.8)' : 'rgba(238,238,238,0.3)',
            }}>{label}</button>
          ))}
        </div>
        ...
        <div style={{ flex: 1, overflowY: 'auto', padding: '0 20px 20px' }}>
          {/* New task form */}
          {showForm && (
            <div style={{ ...card, flexDirection: 'column', gap: 8, marginBottom: 12, borderColor: 'rgba(255,224,194,0.15)' }}>
              ...
              <div style={{ display: 'flex', gap: 8 }}>
                <button onClick={save} style={{ padding: '7px 20px', borderRadius: 8, background: 'rgba(255,224,194,0.12)', border: '1px solid rgba(255,224,194,0.2)', color: 'rgba(255,224,194,0.8)', fontSize: 13, cursor: 'pointer', fontFamily: T.font }}>Save</button>
                <button onClick={() => setShowForm(false)} style={{ padding: '7px 14px', borderRadius: 8, background: 'transparent', border: `0.5px solid rgba(238,238,238,0.06)`, color: T.textTertiary, fontSize: 13, cursor: 'pointer', fontFamily: T.font }}>Cancel</button>
              </div>
            </div>
          )}
          ...
          {filtered.map(task => {
            ...
            return (
              <div key={task.id} onClick={() => getKikoRec(task)} style={{
                ...card,
                borderColor: isSelected ? 'rgba(255,224,194,0.3)' : 'rgba(238,238,238,0.04)',
                background: isSelected ? 'rgba(255,224,194,0.03)' : 'rgba(238,238,238,0.02)',
              }}
                onMouseEnter={e => { if (!isSelected) { e.currentTarget.style.background = 'rgba(238,238,238,0.04)'; e.currentTarget.style.borderColor = 'rgba(238,238,238,0.08)' }}}
                onMouseLeave={e => { if (!isSelected) { e.currentTarget.style.background = 'rgba(238,238,238,0.02)'; e.currentTarget.style.borderColor = 'rgba(238,238,238,0.04)' }}}
              >
                {/* Priority bar */}
                ...
                <div style={{ flex: 1, minWidth: 0 }}>
                  ...
                  <div style={{ fontSize: 14, fontWeight: 400, color: d.completed ? T.textTertiary : 'rgba(238,238,238,0.8)', textDecoration: d.completed ? 'line-through' : 'none', marginBottom: 4, lineHeight: 1.4 }}>{d.notes || d.type || 'Untitled'}</div>
                ...
                {/* Delete */}
                <button onClick={e => { e.stopPropagation(); deleteTask(task.id) }} style={{ background: 'none', border: 'none', cursor: 'pointer', color: 'rgba(238,238,238,0.08)', flexShrink: 0, padding: 0 }}
                  onMouseEnter={e => e.currentTarget.style.color = 'rgba(255,59,48,0.4)'}
                  onMouseLeave={e => e.currentTarget.style.color = 'rgba(238,238,238,0.08)'}
                ><X size={13} /></button>
              </div>
            )
          })}
      ...
      <div style={{ flex: 1, borderLeft: `1px solid rgba(238,238,238,0.04)`, display: 'flex', flexDirection: 'column', background: 'rgba(238,238,238,0.01)', flexShrink: 0, minWidth: 0 }}>
        ...
        <div style={{ padding: '14px 16px', borderBottom: '1px solid rgba(238,238,238,0.04)', display: 'flex', alignItems: 'center', gap: 8 }}>
          ...
          <span style={{ fontSize: 12, fontWeight: 500, color: 'rgba(255,224,194,0.6)', letterSpacing: '0.04em' }}>Kiko Recommendation</span>
        </div>
        ...
        <div style={{ flex: 1, overflowY: 'auto', padding: 16 }}>
          ...
          {kikoLoading && (
            <div style={{ display: 'flex', flexDirection: 'column', alignItems: 'center', gap: 12, padding: '40px 0' }}>
              ...
              <span style={{ fontSize: 13, color: 'rgba(255,224,194,0.6)', fontWeight: 400 }}>Analysing task...</span>
            </div>
          )}
          ...
          {kikoRec && selected && (
            <div style={{ fontSize: 14, color: 'rgba(238,238,238,0.75)', fontWeight: 300, lineHeight: 1.65 }}>
              {/* Task context header */}
              <div style={{ marginBottom: 12, padding: '8px 10px', borderRadius: 8, background: 'rgba(238,238,238,0.02)', border: '1px solid rgba(238,238,238,0.04)' }}>
                ...
                <div style={{ fontSize: 13, color: 'rgba(238,238,238,0.6)', fontWeight: 400 }}>{selected.data?.company}{selected.data?.contact ? ` · ${selected.data.contact}` : ''}</div>
              </div>
