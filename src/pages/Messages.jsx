            ...
            <div style={{ display: 'flex', gap: 6 }}>
              <button onClick={() => {
                ...
                if (!otherPresence || otherPresence === 'offline') {
                  setOfflineCallConfirm({ recipientId: otherMember, recipientName: member?.name || getChannelDisplayName(activeChannelData) })
                } else {
                  voice.startCall(otherMember, member?.name || getChannelDisplayName(activeChannelData))
                }
              }} style={{ height: 32, padding: '0 14px', borderRadius: 8, background: C.green, border: 'none', color: '#fff', fontSize: 12, fontWeight: 600, cursor: 'pointer', display: 'flex', alignItems: 'center', gap: 5, fontFamily: C.font }}><Icon name="phone" size={14} color="#fff" /> Audio</button>
              <button onClick={() => {
                const otherMember = activeChannelData.members?.find(m => m !== userId)
                const member = TEAM_MEMBERS.find(m => m.id === otherMember)
                voice.startCall(otherMember, member?.name || getChannelDisplayName(activeChannelData), true)
              }} style={{ height: 32, padding: '0 14px', borderRadius: 8, background: '#7C3AED', border: 'none', color: '#fff', fontSize: 12, fontWeight: 600, cursor: 'pointer', display: 'flex', alignItems: 'center', gap: 5, fontFamily: C.font }}><svg width="14" height="14" viewBox="0 0 24 24" fill="none" stroke="#fff" strokeWidth="2"><rect x="2" y="6" width="14" height="12" rx="2"/><path d="m16 10 5-3v10l-5-3"/></svg> Video</button>
              ...
              <button onClick={() => { const title = prompt('Meeting title:'); if (title) { const time = prompt('Date & time (YYYY-MM-DDTHH:MM):'); if (time) { fetch(`${API}/api/team-messages?action=schedule-meeting`, { method: 'POST', headers: { 'Content-Type': 'application/json' }, body: JSON.stringify({ title, startTime: time.includes('T') ? time + ':00' : time + 'T09:00:00', channelId: activeChannel, fromUserId: userId, fromName: userName }) }).then(r => r.json()).then(d => { if (d.success) { loadMessages() } else { alert(d.error || 'Failed') } }).catch(e => alert(e.message)) } } }} style={{ height: 32, padding: '0 10px', borderRadius: 8, background: C.card, border: `1px solid ${C.border}`, color: C.sub, fontSize: 12, fontWeight: 500, cursor: 'pointer', display: 'flex', alignItems: 'center', gap: 4, fontFamily: C.font }}>📅 Meet</button>
                ...
                <div style={{ maxWidth: '52%', position: 'relative' }}>
                  ...
                  {hoveredMsg === msg.id && !isDeleted && (
                    <div style={{ position: 'absolute', top: -4, [isMine ? 'left' : 'right']: 0, display: 'flex', gap: 2, background: C.bg, border: `1px solid ${C.border}`, borderRadius: 8, padding: '2px 4px', boxShadow: '0 2px 8px rgba(0,0,0,0.08)', zIndex: 10 }}>
                      ...
                      <button onClick={() => { const targetCh = prompt('Forward to channel (enter channel name):\n' + channels.map(c => c.name || c.id).join(', ')); if (targetCh) { const ch = channels.find(c => (c.name || c.id).toLowerCase() === targetCh.toLowerCase()); if (ch) { fetch(`${API}/api/team-messages?action=forward`, { method: 'POST', headers: { 'Content-Type': 'application/json' }, body: JSON.stringify({ messageId: msg.id, targetChannelId: ch.id, fromUserId: userId, fromName: userName }) }).catch(() => {}) } else { alert('Channel not found') } } }} style={{ background: 'none', border: 'none', cursor: 'pointer', fontSize: 12, padding: '2px 4px' }} title="Forward"><svg width="14" height="14" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="1.8" strokeLinecap="round" strokeLinejoin="round"><polyline points="15 17 20 12 15 7"/><path d="M4 18v-2a4 4 0 014-4h12"/></svg></button>
