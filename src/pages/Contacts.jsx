    ...
    <div className="flex-1 overflow-hidden flex flex-col">
      <div className="px-6 py-4 border-b border-white/8 flex items-center justify-between">
        <div>
          ...
          <p className="text-xs text-white/30 mt-0.5">{filtered.length.toLocaleString()} contact{filtered.length !== 1 ? 's' : ''}</p>
        </div>
      ...
      <div className="px-6 py-3 border-b border-white/8 flex items-center gap-4">
        <div className="relative flex-1 max-w-sm">
          ...
          <input value={search} onChange={e => setSearch(e.target.value)} placeholder="Search contacts..." className="w-full bg-white/5 border border-white/10 rounded-lg pl-9 pr-4 py-2 text-sm text-white placeholder:text-white/25 outline-none focus:border-white/25 transition-colors" />
        </div>
        {totalPages > 1 && (
