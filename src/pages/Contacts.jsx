...
export default function Contacts({ user }) {
  ...
  const glass = { padding: '12px 20px', borderRadius: 20, background: 'rgba(25,25,25,0.40)', backdropFilter: 'blur(40px) saturate(1.6)', WebkitBackdropFilter: 'blur(40px) saturate(1.6)', border: '0.5px solid rgba(0,0,0,0.08)', boxShadow: 'inset 0 1px 0 rgba(0,0,0,0.06), 0 8px 32px rgba(0,0,0,0.35)', display: 'flex', alignItems: 'center', justifyContent: 'space-between' }
  const inputStyle = { width: '100%', background: 'rgba(25,25,25,0.40)', border: '0.5px solid rgba(0,0,0,0.08)', borderRadius: 50, padding: '10px 14px', fontSize: 14, color: 'var(--text)', outline: 'none', fontFamily: 'var(--font)', boxSizing: 'border-box' }
  ...
  const actionBtn = { width: 30, height: 30, borderRadius: 50, background: 'rgba(25,25,25,0.40)', border: '0.5px solid rgba(0,0,0,0.08)', display: 'flex', alignItems: 'center', justifyContent: 'center', color: '#A0A0A0', cursor: 'pointer', transition: 'all 0.2s', boxShadow: 'inset 0 1px 0 rgba(25,25,25,0.35)', flexShrink: 0 }
  const stageColors = { 'To revisit': ['rgba(25,25,25,0.40)','rgba(0,0,0,0.05)','rgba(0,0,0,0.10)'], 'Contact made': ['rgba(0,0,0,0.05)','rgba(0,0,0,0.08)','rgba(124,92,252,0.55)'], 'In Dialogue': ['rgba(245,158,11,0.08)','rgba(245,158,11,0.15)','rgba(245,158,11,0.6)'], 'Qualified': ['rgba(6,214,160,0.08)','rgba(6,214,160,0.15)','rgba(6,214,160,0.55)'], 'Meeting arranged (brand x RH)': ['rgba(59,130,246,0.08)','rgba(59,130,246,0.15)','rgba(59,130,246,0.55)'] }
  const avatarColors = ['rgba(0,0,0,0.08)', 'rgba(6,214,160,0.15)', 'rgba(236,72,153,0.15)', 'rgba(59,130,246,0.15)', 'rgba(245,158,11,0.15)']
    ...
    <div style={{ flex: 1, display: 'flex', flexDirection: 'column', overflow: 'hidden' }}>
      ...
      <div style={{ padding: isRedesignOn() ? '8px 24px 0' : '20px 24px 0', display: 'flex', alignItems: 'center', justifyContent: 'space-between' }}>
        ...
        <div style={{ display: 'flex', gap: 8, alignItems: 'center' }}>
          <div style={{ display: 'flex', alignItems: 'center', width: 260, background: 'rgba(25,25,25,0.40)', backdropFilter: 'blur(40px) saturate(1.6)', border: '0.5px solid rgba(0,0,0,0.08)', borderRadius: 50, padding: '0 16px', boxShadow: 'inset 0 2px 0 rgba(0,0,0,0.06), inset 0 -1px 0 rgba(0,0,0,0.05)' }}>
            ...
            <input value={search} onChange={e => setSearch(e.target.value)} placeholder="Search contacts..." style={{ flex: 1, border: 'none', background: 'transparent', outline: 'none', fontSize: 14, color: '#6B6B6B', fontFamily: 'var(--font)', height: 38, fontWeight: 300 }} />
          </div>
          <select value={sortDir} onChange={e => setSortDir(e.target.value)} style={pillBtn('rgba(0,0,0,0.05)','rgba(0,0,0,0.08)','rgba(124,92,252,0.65)')}>
      ...
      <div style={{ flex: 1, padding: '16px 24px', overflow: 'hidden' }}>
        <div style={{ flex: 1, overflowY: 'auto', display: 'flex', flexDirection: 'column', gap: 3, height: '100%' }}>
          {/* Column headers */}
          <div style={{ display: 'flex', alignItems: 'center', gap: 14, padding: '6px 18px 8px', fontSize: 11, color: '#6B6B6B', textTransform: 'uppercase', letterSpacing: '0.06em', fontWeight: 300, fontFamily: 'var(--font)' }}>
          ...
          {loading ? (
            ...
            <div style={{ display: 'flex', flexDirection: 'column', alignItems: 'center', justifyContent: 'center', flex: 1, color: '#A0A0A0' }}>
            ...
            <>
              {paged.map(contact => {
                ...
                return (
                  <div key={contact.id} onClick={() => nav(`/contacts/${contact.id}`)}
                    style={{ display: 'flex', alignItems: 'center', gap: 14, padding: '10px 18px', borderRadius: 12, background: 'rgba(25,25,25,0.55)', border: '0.5px solid rgba(0,0,0,0.08)', cursor: 'pointer', transition: 'all 0.15s' }}
                    onMouseEnter={e => { e.currentTarget.style.background = 'rgba(0,0,0,0.04)'; e.currentTarget.style.borderColor = 'rgba(0,0,0,0.06)' }}
                    onMouseLeave={e => { e.currentTarget.style.background = 'rgba(25,25,25,0.55)'; e.currentTarget.style.borderColor = 'rgba(0,0,0,0.08)' }}>
                    ...
                    <div style={{ width: 180, fontSize: 14, fontWeight: 400, color: '#0A0A0A', fontFamily: 'var(--font)', overflow: 'hidden', textOverflow: 'ellipsis', whiteSpace: 'nowrap' }}>{displayName(contact)}</div>
                    <div style={{ width: 150, fontSize: 12, color: '#6B6B6B', fontFamily: 'var(--font)', overflow: 'hidden', textOverflow: 'ellipsis', whiteSpace: 'nowrap' }}>{contact.company || '—'}</div>
                    <div style={{ width: 180, fontSize: 12, color: '#6B6B6B', fontFamily: 'var(--font)', overflow: 'hidden', textOverflow: 'ellipsis', whiteSpace: 'nowrap' }}>{contact.title || '—'}</div>
                    <div style={{ flex: 1, fontSize: 12, color: '#6B6B6B', fontFamily: 'var(--font)', overflow: 'hidden', textOverflow: 'ellipsis', whiteSpace: 'nowrap' }}>{contact.email || ''}</div>
                    {/* Quick actions */}
              ...
              {totalPages > 1 && (
                <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'center', gap: 8, padding: '12px 0', fontSize: 13, color: '#6B6B6B', fontFamily: 'var(--font)' }}>
                  <button disabled={page === 0} onClick={() => setPage(p => p - 1)} style={{ background: 'none', border: 'none', cursor: page === 0 ? 'default' : 'pointer', opacity: page === 0 ? 0.3 : 1, color: '#6B6B6B', padding: 4 }}><ChevronLeft style={{ width: 16, height: 16 }} /></button>
                  <span>{page + 1} / {totalPages}</span>
                  <button disabled={page >= totalPages - 1} onClick={() => setPage(p => p + 1)} style={{ background: 'none', border: 'none', cursor: page >= totalPages - 1 ? 'default' : 'pointer', opacity: page >= totalPages - 1 ? 0.3 : 1, color: '#6B6B6B', padding: 4 }}><ChevronRight style={{ width: 16, height: 16 }} /></button>
                </div>
              )}
            </>
          )}
      ...
      {showForm && (
        <div onClick={e => e.target === e.currentTarget && reset()} style={{ position: 'fixed', inset: 0, background: 'rgba(0,0,0,0.6)', backdropFilter: 'blur(12px)', zIndex: 50, display: 'flex', alignItems: 'center', justifyContent: 'center', padding: 16 }}>
          <div style={{ background: 'rgba(25,25,25,0.50)', backdropFilter: 'blur(40px)', WebkitBackdropFilter: 'blur(40px)', borderRadius: 24, border: '0.5px solid rgba(0,0,0,0.08)', boxShadow: 'inset 0 1px 0 rgba(0,0,0,0.05), 0 24px 80px rgba(0,0,0,0.5)', width: '100%', maxWidth: 420, padding: 24 }}>
            <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', marginBottom: 20 }}>
              <h2 style={{ fontSize: 17, fontWeight: 200, color: '#0A0A0A', margin: 0, fontFamily: 'var(--font)' }}>{editing ? 'Edit Contact' : 'Add Contact'}</h2>
            ...
            <div style={{ display: 'flex', gap: 10, marginTop: 20 }}>
              ...
              <button onClick={save} style={{ flex: 1, padding: '10px 0', fontSize: 14, color: '#0A0A0A', background: 'var(--accent)', border: 'none', borderRadius: 50, cursor: 'pointer', fontWeight: 500, fontFamily: 'var(--font)' }}>Save</button>
            </div>
          </div>
        </div>
      )}
