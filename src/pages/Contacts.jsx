...
export default function Contacts({ user }) {
  ...
  const glass = { padding: '12px 20px', borderRadius: 20, background: 'rgba(25,25,25,0.40)', backdropFilter: 'blur(40px) saturate(1.6)', WebkitBackdropFilter: 'blur(40px) saturate(1.6)', border: '0.5px solid rgba(32,30,24,0.50)', boxShadow: 'inset 0 1px 0 rgba(255,224,194,0.10), 0 8px 32px rgba(0,0,0,0.35)', display: 'flex', alignItems: 'center', justifyContent: 'space-between' }
  const inputStyle = { width: '100%', background: 'rgba(25,25,25,0.40)', border: '0.5px solid rgba(32,30,24,0.50)', borderRadius: 50, padding: '10px 14px', fontSize: 14, color: 'var(--text)', outline: 'none', fontFamily: 'var(--font)', boxSizing: 'border-box' }
  ...
  const actionBtn = { width: 30, height: 30, borderRadius: 50, background: 'rgba(25,25,25,0.40)', border: '0.5px solid rgba(32,30,24,0.50)', display: 'flex', alignItems: 'center', justifyContent: 'center', color: 'rgba(255,224,194,0.15)', cursor: 'pointer', transition: 'all 0.2s', boxShadow: 'inset 0 1px 0 rgba(25,25,25,0.35)', flexShrink: 0 }
  const stageColors = { 'To revisit': ['rgba(25,25,25,0.40)','rgba(255,224,194,0.08)','rgba(255,224,194,0.25)'], 'Contact made': ['rgba(255,224,194,0.08)','rgba(255,224,194,0.15)','rgba(255,224,194,0.55)'], 'In Dialogue': ['rgba(245,158,11,0.08)','rgba(245,158,11,0.15)','rgba(245,158,11,0.6)'], 'Qualified': ['rgba(6,214,160,0.08)','rgba(6,214,160,0.15)','rgba(6,214,160,0.55)'], 'Meeting arranged (brand x RH)': ['rgba(59,130,246,0.08)','rgba(59,130,246,0.15)','rgba(59,130,246,0.55)'] }
    ...
    <div style={{ flex: 1, display: 'flex', flexDirection: 'column', overflow: 'hidden' }}>
      ...
      <div style={{ padding: '20px 24px 0', display: 'flex', alignItems: 'center', justifyContent: 'space-between' }}>
        <div>
          ...
          <div style={{ fontSize: 14, color: 'rgba(255,224,194,0.25)', fontWeight: 300, marginTop: 2, fontFamily: 'var(--font)' }}>{filtered.length.toLocaleString()} contacts</div>
        </div>
        <div style={{ display: 'flex', gap: 8, alignItems: 'center' }}>
          <div style={{ display: 'flex', alignItems: 'center', width: 260, background: 'rgba(25,25,25,0.40)', backdropFilter: 'blur(40px) saturate(1.6)', border: '0.5px solid rgba(32,30,24,0.50)', borderRadius: 50, padding: '0 16px', boxShadow: 'inset 0 2px 0 rgba(255,224,194,0.10), inset 0 -1px 0 rgba(255,224,194,0.08)' }}>
            <Search style={{ width: 14, height: 14, color: 'rgba(255,224,194,0.25)', flexShrink: 0, marginRight: 8 }} />
      ...
      <div style={{ flex: 1, padding: '16px 24px', overflow: 'hidden' }}>
        <div style={{ flex: 1, overflowY: 'auto', display: 'flex', flexDirection: 'column', gap: 3, height: '100%' }}>
          ...
          {loading ? (
            ...
            <div style={{ display: 'flex', flexDirection: 'column', alignItems: 'center', justifyContent: 'center', flex: 1, color: 'rgba(255,224,194,0.15)' }}>
            ...
            <>
              {paged.map(contact => {
                ...
                return (
                  <div key={contact.id} onClick={() => nav(`/contacts/${contact.id}`)}
                    style={{ display: 'flex', alignItems: 'center', gap: 14, padding: '10px 18px', borderRadius: 12, background: 'rgba(25,25,25,0.55)', border: '0.5px solid rgba(32,30,24,0.50)', cursor: 'pointer', transition: 'all 0.15s' }}
                    ...
                    onMouseLeave={e => { e.currentTarget.style.background = 'rgba(25,25,25,0.55)'; e.currentTarget.style.borderColor = 'rgba(32,30,24,0.50)' }}>
                    ...
                    <div style={{ width: 180, fontSize: 14, fontWeight: 400, color: 'rgba(238,232,220,0.82)', fontFamily: 'var(--font)', overflow: 'hidden', textOverflow: 'ellipsis', whiteSpace: 'nowrap' }}>{displayName(contact)}</div>
                    <div style={{ width: 150, fontSize: 12, color: 'rgba(255,224,194,0.30)', fontFamily: 'var(--font)', overflow: 'hidden', textOverflow: 'ellipsis', whiteSpace: 'nowrap' }}>{contact.company || '—'}</div>
                    <div style={{ width: 180, fontSize: 12, color: 'rgba(255,224,194,0.35)', fontFamily: 'var(--font)', overflow: 'hidden', textOverflow: 'ellipsis', whiteSpace: 'nowrap' }}>{contact.title || '—'}</div>
                    <div style={{ flex: 1, fontSize: 12, color: 'rgba(255,224,194,0.30)', fontFamily: 'var(--font)', overflow: 'hidden', textOverflow: 'ellipsis', whiteSpace: 'nowrap' }}>{contact.email || ''}</div>
                    {/* Quick actions */}
              ...
              {totalPages > 1 && (
                <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'center', gap: 8, padding: '12px 0', fontSize: 13, color: 'rgba(255,224,194,0.25)', fontFamily: 'var(--font)' }}>
                  <button disabled={page === 0} onClick={() => setPage(p => p - 1)} style={{ background: 'none', border: 'none', cursor: page === 0 ? 'default' : 'pointer', opacity: page === 0 ? 0.3 : 1, color: 'rgba(255,224,194,0.35)', padding: 4 }}><ChevronLeft style={{ width: 16, height: 16 }} /></button>
                  <span>{page + 1} / {totalPages}</span>
                  <button disabled={page >= totalPages - 1} onClick={() => setPage(p => p + 1)} style={{ background: 'none', border: 'none', cursor: page >= totalPages - 1 ? 'default' : 'pointer', opacity: page >= totalPages - 1 ? 0.3 : 1, color: 'rgba(255,224,194,0.35)', padding: 4 }}><ChevronRight style={{ width: 16, height: 16 }} /></button>
                </div>
              )}
            </>
          )}
      ...
      {showForm && (
        <div onClick={e => e.target === e.currentTarget && reset()} style={{ position: 'fixed', inset: 0, background: 'rgba(0,0,0,0.6)', backdropFilter: 'blur(12px)', zIndex: 50, display: 'flex', alignItems: 'center', justifyContent: 'center', padding: 16 }}>
          <div style={{ background: 'rgba(25,25,25,0.50)', backdropFilter: 'blur(40px)', WebkitBackdropFilter: 'blur(40px)', borderRadius: 24, border: '0.5px solid rgba(32,30,24,0.50)', boxShadow: 'inset 0 1px 0 rgba(255,224,194,0.08), 0 24px 80px rgba(0,0,0,0.5)', width: '100%', maxWidth: 420, padding: 24 }}>
