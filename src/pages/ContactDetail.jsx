...
export default function ContactDetail() {
  ...
  const glass = { background: 'rgba(124,92,252,0.04)', backdropFilter: 'blur(40px)', WebkitBackdropFilter: 'blur(40px)', border: '0.5px solid #26262f', boxShadow: '0 8px 36px rgba(0,0,0,0.3)' }
  const card = { background: 'rgba(124,92,252,0.04)', borderRadius: 18, padding: 24, border: '0.5px solid #26262f', boxShadow: 'none' }
  const inputStyle = { width: '100%', background: 'rgba(124,92,252,0.04)', border: '1px solid var(--border)', borderRadius: 50, padding: '10px 14px', fontSize: 14, color: 'var(--text)', outline: 'none', fontFamily: 'var(--font)', boxSizing: 'border-box' }
  ...
  if (loading) return <div style={{ flex: 1, display: 'flex', alignItems: 'center', justifyContent: 'center' }}><div style={{ width: 24, height: 24, border: '2px solid #26262f', borderTopColor: 'var(--accent)', borderRadius: '50%', animation: 'spin 0.8s linear infinite' }} /></div>
    ...
    <div style={{ flex: 1, display: 'flex', flexDirection: 'column', overflow: 'hidden', paddingTop: 8 }}>
      ...
      <div style={{ margin: '0 16px', padding: '10px 20px', borderRadius: 18, ...glass, display: 'flex', alignItems: 'center', justifyContent: 'space-between' }}>
        <div style={{ display: 'flex', alignItems: 'center', gap: 12 }}>
          <button onClick={() => nav('/contacts')} style={{ background: 'rgba(124,92,252,0.04)', border: 'none', borderRadius: 50, width: 32, height: 32, display: 'flex', alignItems: 'center', justifyContent: 'center', cursor: 'pointer', color: 'var(--text-secondary)' }}>
          ...
          <div>
            ...
            {campaignHistory.length > 0 && (
              <div style={{ display: 'flex', gap: 6, marginTop: 8, flexWrap: 'wrap' }}>
                <span style={{ display: 'inline-flex', alignItems: 'center', gap: 4, padding: '3px 9px', borderRadius: 50, background: 'rgba(124,92,252,0.10)', border: '0.5px solid rgba(124,92,252,0.20)', color: '#7c5cfc', fontSize: 10, fontWeight: 500, fontFamily: 'var(--font)' }}>
        ...
        <div style={{ width: 340, flexShrink: 0, display: 'flex', flexDirection: 'column', gap: 16 }}>
          ...
          <div style={card}>
            <div style={{ display: 'flex', alignItems: 'center', gap: 16, marginBottom: 20 }}>
              {contact.picture ? (
                <img src={contact.picture} alt="" style={{ width: 56, height: 56, borderRadius: 18, objectFit: 'cover' }} onError={e => { e.target.style.display = 'none'; e.target.parentElement.innerHTML = `<span style="font-size:24px;font-weight:600;color:rgba(124,92,252,0.7)">${(contact.firstName || '?')[0]?.toUpperCase()}${(contact.lastName || '')[0]?.toUpperCase() || ''}</span>` }} />
              ) : (
                <div style={{ width: 56, height: 56, borderRadius: 18, background: 'rgba(124,92,252,0.04)', display: 'flex', alignItems: 'center', justifyContent: 'center' }}>
            ...
            <div style={{ display: 'flex', gap: 8, flexWrap: 'wrap' }}>
              {contact.email && <a href={`mailto:${contact.email}`} style={{ display: 'flex', alignItems: 'center', gap: 6, fontSize: 13, color: 'var(--text-secondary)', background: 'rgba(124,92,252,0.04)', padding: '6px 12px', borderRadius: 50, textDecoration: 'none', fontFamily: 'var(--font)', border: '0.5px solid #26262f' }}><Mail style={{ width: 13, height: 13 }} /> Email</a>}
              {contact.phone && <a href={`tel:${contact.phone}`} style={{ display: 'flex', alignItems: 'center', gap: 6, fontSize: 13, color: 'var(--text-secondary)', background: 'rgba(124,92,252,0.04)', padding: '6px 12px', borderRadius: 50, textDecoration: 'none', fontFamily: 'var(--font)', border: '0.5px solid #26262f' }}><Phone style={{ width: 13, height: 13 }} /> Call</a>}
              {contact.linkedin && <a href={contact.linkedin} target="_blank" rel="noopener noreferrer" style={{ display: 'flex', alignItems: 'center', gap: 6, fontSize: 13, color: 'var(--text-secondary)', background: 'rgba(124,92,252,0.04)', padding: '6px 12px', borderRadius: 50, textDecoration: 'none', fontFamily: 'var(--font)', border: '0.5px solid #26262f' }}><Linkedin style={{ width: 13, height: 13 }} /> LinkedIn</a>}
            </div>
          </div>
            ...
            {editing ? (
              <>
                ...
                <div style={{ display: 'flex', flexDirection: 'column', gap: 12 }}>
                  ...
                  <button onClick={save} style={{ alignSelf: 'flex-end', fontSize: 14, fontWeight: 500, background: 'var(--accent)', color: '#f4f4f6', padding: '8px 20px', borderRadius: 50, border: 'none', cursor: 'pointer', fontFamily: 'var(--font)' }}>Save Changes</button>
                </div>
              </>
            ) : (
              <>
                ...
                {contact.notes && contact.notes !== '[]' && (
                  <div style={{ marginTop: 24, paddingTop: 20, borderTop: '1px solid rgba(124,92,252,0.04)' }}>
