...
function statusColor(c) {
  ...
  return { bg: 'rgba(25,25,25,0.40)', color: T.textTertiary, border: 'rgba(167,139,250,0.06)' }
}

function stepIcon(type) {
  const t = (type || '').toLowerCase()
  if (t.includes('email') || t.includes('mail')) return <Mail size={14} style={{ color: 'rgba(167,139,250,0.7)' }} />
  ...
  if (t.includes('delay') || t.includes('wait')) return <Clock size={14} style={{ color: 'rgba(167,139,250,0.35)' }} />
  return <Send size={14} style={{ color: 'rgba(167,139,250,0.45)' }} />
}

function activityIcon(type) {
  const t = (type || '').toLowerCase()
  if (t.includes('replied')) return <Reply size={12} style={{ color: 'rgba(6,214,160,0.8)' }} />
  if (t.includes('opened') || t.includes('open')) return <Eye size={12} style={{ color: 'rgba(167,139,250,0.6)' }} />
  ...
  if (t.includes('sent')) return <Mail size={12} style={{ color: 'rgba(167,139,250,0.45)' }} />
  return <Mail size={12} style={{ color: 'rgba(167,139,250,0.30)' }} />
}
...
export default function Lemlist({ user }) {
  ...
  const loadCampaigns = async () => {
    ...
    try {
      ...
      const { data: enrolls } = await supabase.from('kiko_sequence_enrollments').select('sequence_id, status')
      // Map our schema to Lemlist-shaped objects so the JSX renders unchanged
      const enrollCounts = {}
      ;(enrolls || []).forEach(e => { enrollCounts[e.sequence_id] = (enrollCounts[e.sequence_id] || 0) + 1 })
      const arr = (seqs || []).map(s => ({
        _id: s.id,
        name: s.name,
        state: s.is_active ? 'running' : (enrollCounts[s.id] > 0 ? 'paused' : 'draft'),
        createdAt: s.created_at,
        _raw: s, // keep original for selectCampaign
      }))
      arr.sort((a, b) => {
      ...
      setCampaigns(arr)
      setPageContext({ page: 'campaigns', summary: `Campaigns: ${arr.length} sequences`, visibleItems: arr.slice(0, 8).map(x => `${x.name} (${statusLabel(x)})`).join(', ') })
  ...
  const selectCampaign = async (c) => {
    ...
    setSequenceSteps([])
    try {
      // Fetch enrollments + queue rows for this sequence
      const [{ data: enr }, { data: queue }] = await Promise.all([
        supabase.from('kiko_sequence_enrollments').select('*').eq('sequence_id', c._id).order('created_at', { ascending: false }),
        supabase.from('kiko_outreach_queue').select('*').in('enrollment_id', []).order('scheduled_for', { ascending: false }),
      ])
      const enrollments = enr || []
      const enrollmentIds = enrollments.map(e => e.id)
      // Now fetch queue with the IDs we have
      let q = []
      if (enrollmentIds.length > 0) {
        const { data: qData } = await supabase.from('kiko_outreach_queue').select('*').in('enrollment_id', enrollmentIds).order('updated_at', { ascending: false })
        q = qData || []
      }

      // Build stats from queue rows
      const sent = q.filter(x => x.status === 'sent').length
      const opened = q.filter(x => x.opened_at).length
      const clicked = q.filter(x => x.clicked_at).length
      const replied = enrollments.filter(e => e.status === 'replied' || e.reply_detected_at).length
      const bounced = q.filter(x => x.status === 'bounced').length
      setStats({ sent, opened, clicked, replied, bounced, sentCount: sent, openCount: opened, clickCount: clicked, replyCount: replied, bouncedCount: bounced })

      // Activities feed: combine queue rows + reply events
      const acts = []
      for (const row of q) {
        const enr = enrollments.find(e => e.id === row.enrollment_id)
        const lead = enr ? { email: enr.contact_email, firstName: (enr.contact_name || '').split(' ')[0], lastName: (enr.contact_name || '').split(' ').slice(1).join(' '), companyName: enr.company } : {}
        if (row.sent_at) acts.push({ type: 'emailsSent', createdAt: row.sent_at, leadEmail: lead.email, leadFirstName: lead.firstName, leadLastName: lead.lastName, leadCompanyName: lead.companyName, leadId: row.enrollment_id, stepNumber: row.step_number, subject: row.subject })
        if (row.opened_at) acts.push({ type: 'emailsOpened', createdAt: row.opened_at, leadEmail: lead.email, leadFirstName: lead.firstName, leadLastName: lead.lastName, leadCompanyName: lead.companyName, leadId: row.enrollment_id, stepNumber: row.step_number })
        if (row.clicked_at) acts.push({ type: 'emailsClicked', createdAt: row.clicked_at, leadEmail: lead.email, leadFirstName: lead.firstName, leadLastName: lead.lastName, leadCompanyName: lead.companyName, leadId: row.enrollment_id, stepNumber: row.step_number })
      }
      for (const e of enrollments) {
        if (e.reply_detected_at) {
          acts.push({ type: 'emailsReplied', createdAt: e.reply_detected_at, leadEmail: e.contact_email, leadFirstName: (e.contact_name || '').split(' ')[0], leadLastName: (e.contact_name || '').split(' ').slice(1).join(' '), leadCompanyName: e.company, leadId: e.id })
        }
      }
      acts.sort((a, b) => new Date(b.createdAt || 0) - new Date(a.createdAt || 0))
      setCampaignActivities(acts)

      // Leads list from enrollments (always show all enrolled, not just those with activity)
      const leadsList = enrollments.map(e => ({
        _id: e.id,
        email: e.contact_email,
        firstName: (e.contact_name || '').split(' ')[0] || '',
        lastName: (e.contact_name || '').split(' ').slice(1).join(' ') || '',
        companyName: e.company || '',
        lastActivity: e.updated_at || e.created_at,
        lastType: e.status,
        activities: q.filter(x => x.enrollment_id === e.id).length,
      })).sort((a, b) => new Date(b.lastActivity || 0) - new Date(a.lastActivity || 0))
      setLeads(leadsList)

      // Sequence steps from the seq.steps JSONB
      const rawSteps = c._raw?.steps || []
      const stepsList = rawSteps.map((s, i) => ({
        _id: `step_${i + 1}`,
        stepNumber: s.step || (i + 1),
        type: s.channel || (s.type === 'condition' ? 'condition' : 'email'),
        subject: s.subject || '',
        delay_days: s.delay_days || 0,
        count: q.filter(x => x.step_number === (s.step || i + 1) && x.status === 'sent').length,
      }))
      setSequenceSteps(stepsList)
    } catch (e) { console.error('[Campaigns detail]', e) }
  ...
  const sectionTitle = { fontSize: 12, fontWeight: 500, color: 'rgba(167,139,250,0.45)', textTransform: 'uppercase', letterSpacing: '0.06em', margin: '0 0 10px', fontFamily: T.font }
          ...
          {(() => {
            ...
            return campaigns.map(c => {
              ...
              return (
                <div key={c._id}>
                  {showHeader && (
                    <div style={{ fontSize: 10, fontWeight: 600, color: 'rgba(167,139,250,0.30)', textTransform: 'uppercase', letterSpacing: '0.1em', padding: '12px 12px 6px', marginTop: grp > 0 ? 8 : 0 }}>
                  ...
                  <div onClick={() => selectCampaign(c)} style={{
                    ...
                    background: isActive ? 'rgba(167,139,250,0.06)' : 'rgba(238,238,238,0.015)',
                    border: `1px solid ${isActive ? 'rgba(167,139,250,0.15)' : 'rgba(25,25,25,0.35)'}`,
                  }}
                  ...
                  >
                    ...
                    <div style={{ display: 'flex', gap: 6, alignItems: 'center' }}>
                      ...
                      <span style={{ fontSize: 10, color: 'rgba(167,139,250,0.15)', marginLeft: 'auto' }}>{timeAgo(c.createdAt)}</span>
                    </div>
                  </div>
                </div>
              )
                ...
                <div style={{ display: 'grid', gridTemplateColumns: 'repeat(6, 1fr)', gap: 8 }}>
                  {[
                    ...
                    <div key={s.label} style={statCard}>
                      <span style={{ fontSize: 10, color: 'rgba(167,139,250,0.35)', textTransform: 'uppercase', letterSpacing: '0.05em' }}>{s.label}</span>
              ...
              {sequenceSteps.length > 0 && (
                <div style={{ marginBottom: 24 }}>
                  ...
                  <div style={{ display: 'flex', flexDirection: 'column', gap: 0, position: 'relative', paddingLeft: 20 }}>
                    {/* Vertical line */}
                    <div style={{ position: 'absolute', left: 6, top: 8, bottom: 8, width: 1, background: 'rgba(167,139,250,0.06)' }} />
                    {sequenceSteps.map((step, idx) => (
                      <div key={step._id || idx} style={{ display: 'flex', gap: 12, alignItems: 'flex-start', padding: '8px 0', position: 'relative' }}>
                        <div style={{ position: 'absolute', left: -17, top: 12, width: 10, height: 10, borderRadius: '50%', background: '#111114', border: '2px solid rgba(167,139,250,0.4)', zIndex: 1 }} />
                        <div style={{ flex: 1, ...card, padding: '10px 14px' }}>
                          ...
                          {step.subject && <div style={{ fontSize: 13, color: 'rgba(167,139,250,0.55)', marginTop: 4 }}>Subject: {step.subject}</div>}
                        </div>
                      </div>
                    ))}
                  </div>
                  ...
                  {leads.map((lead, idx) => {
                    ...
                    return (
                      <div key={lead._id || idx} onClick={() => selectLead(lead)} style={{
                        ...
                        background: isSelected ? 'rgba(167,139,250,0.06)' : card.background,
                        borderColor: isSelected ? 'rgba(167,139,250,0.15)' : 'rgba(25,25,25,0.40)',
                      }}
                      ...
                      >
                        ...
                        <div style={{ display: 'flex', flexDirection: 'column', alignItems: 'flex-end', gap: 2, flexShrink: 0 }}>
                          ...
                          <span style={{ fontSize: 10, color: 'rgba(167,139,250,0.25)' }}>{lead.activities} events · {timeAgo(lead.lastActivity)}</span>
                        </div>
                        <ChevronRight size={14} style={{ color: 'rgba(167,139,250,0.15)', flexShrink: 0 }} />
                      </div>
                    )
                  })}
                </div>
      ...
      {selectedLead && (
        <div style={{ width: 380, borderLeft: '1px solid rgba(25,25,25,0.40)', display: 'flex', flexDirection: 'column', flexShrink: 0, background: 'rgba(238,238,238,0.01)' }}>
          ...
          <div style={{ padding: '14px 16px', borderBottom: '1px solid rgba(25,25,25,0.40)', display: 'flex', alignItems: 'center', gap: 10 }}>
            ...
            <button onClick={() => setSelectedLead(null)} style={{ width: 28, height: 28, borderRadius: 8, background: 'rgba(25,25,25,0.40)', border: 'none', cursor: 'pointer', display: 'flex', alignItems: 'center', justifyContent: 'center', color: 'rgba(167,139,250,0.35)' }}>
              <X size={14} />
            </button>
          </div>
                ...
                {leadActivities.map((a, i) => {
                  ...
                  return (
                    <div key={a._id || i} style={{ display: 'flex', gap: 10, padding: '8px 0', position: 'relative' }}>
                      <div style={{ position: 'absolute', left: -17, top: 12, width: 8, height: 8, borderRadius: '50%', background: '#111114', border: `2px solid ${isReply ? 'rgba(6,214,160,0.5)' : 'rgba(167,139,250,0.3)'}`, zIndex: 1 }} />
                      <div style={{ flex: 1, ...card, padding: '10px 12px', background: isReply ? 'rgba(6,214,160,0.03)' : card.background, borderColor: isReply ? 'rgba(6,214,160,0.1)' : 'rgba(25,25,25,0.40)' }}>
                        <div style={{ display: 'flex', alignItems: 'center', gap: 6, marginBottom: 4 }}>
                          {activityIcon(a.type)}
                          <span style={{ fontSize: 12, fontWeight: 500, color: 'rgba(167,139,250,0.55)', textTransform: 'capitalize' }}>{typeLabel}</span>
                          <span style={{ fontSize: 10, color: 'rgba(167,139,250,0.15)', marginLeft: 'auto' }}>{a.createdAt ? new Date(a.createdAt).toLocaleString('en-GB', { day: 'numeric', month: 'short', hour: '2-digit', minute: '2-digit' }) : ''}</span>
                        </div>
                        ...
                        {/* Campaign name */}
                        {a.campaignName && <div style={{ fontSize: 11, color: 'rgba(167,139,250,0.25)' }}>{a.campaignName}</div>}
                        {/* Message content if sent/replied */}
                        {(isSent || isReply) && a.text && (
                          <div style={{ marginTop: 6, padding: '8px 10px', borderRadius: 8, background: 'rgba(25,25,25,0.30)', border: '0.5px solid rgba(167,139,250,0.50)', fontSize: 12, color: 'rgba(167,139,250,0.45)', lineHeight: 1.55, maxHeight: 120, overflowY: 'auto', whiteSpace: 'pre-wrap' }}>
                            {a.text}
                          </div>
                        )}
                        {(isSent || isReply) && a.html && !a.text && (
                          <div style={{ marginTop: 6, padding: '8px 10px', borderRadius: 8, background: 'rgba(25,25,25,0.30)', border: '0.5px solid rgba(167,139,250,0.50)', fontSize: 12, color: 'rgba(167,139,250,0.45)', lineHeight: 1.55, maxHeight: 120, overflowY: 'auto' }}
