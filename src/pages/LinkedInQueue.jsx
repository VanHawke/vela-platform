        ...
        <div style={{ display: 'flex', gap: 4 }}>
          {[['pending', 'Pending', counts.pending], ['sent', 'Sent', counts.sent], ['skipped', 'Skipped', counts.skipped], ['all', 'All', counts.all]].map(([id, label, count]) => (
            <button key={id} onClick={() => setFilter(id)} style={{
              padding: '7px 14px', borderRadius: 6, border: '1px solid ' + (filter === id ? 'rgba(167,139,250,0.30)' : 'transparent'),
              background: filter === id ? 'rgba(167,139,250,0.10)' : 'transparent',
              color: filter === id ? '#a78bfa' : T.textSecondary, fontSize: 12, fontWeight: 500, cursor: 'pointer', fontFamily: T.font,
            ...
            <div key={item.id} style={{ padding: '16px 18px', borderRadius: 8, border: '1px solid rgba(255,255,255,0.06)', background: 'rgba(255,255,255,0.02)' }}>
              <div style={{ display: 'flex', alignItems: 'flex-start', justifyContent: 'space-between', gap: 16, marginBottom: 10 }}>
                ...
                <div style={{ display: 'flex', gap: 6, alignItems: 'center' }}>
                  ...
                  {item.status === 'sent' && <span style={{ fontSize: 10, padding: '2px 8px', borderRadius: 4, background: 'rgba(45,212,191,0.10)', color: '#2dd4bf', border: '1px solid rgba(45,212,191,0.25)', textTransform: 'uppercase', letterSpacing: '0.04em', fontWeight: 500 }}>Sent</span>}
              ...
              <div style={{ display: 'flex', gap: 8 }}>
                ...
                {item.status === 'pending' && (
                  <>
                    <button onClick={() => markSent(item)} style={{ display: 'inline-flex', alignItems: 'center', gap: 5, padding: '7px 14px', borderRadius: 6, border: '1px solid rgba(45,212,191,0.30)', background: 'rgba(45,212,191,0.08)', color: '#2dd4bf', fontSize: 11, fontWeight: 500, cursor: 'pointer', fontFamily: T.font }}>
