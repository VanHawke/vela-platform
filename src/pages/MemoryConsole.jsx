...
const T = {
  bg: '#000000', surface: 'var(--border)', surfaceHover: 'var(--border)',
  border: 'var(--border)', borderHover: 'var(--border)',
  text: 'var(--foreground)', textSecondary: 'var(--muted-foreground)', textTertiary: 'var(--muted-foreground)',
  accent: 'var(--border)', accentSoft: 'var(--border)',
    ...
    <div style={{ flex: 1, display: 'flex', flexDirection: 'column', background: 'transparent', overflow: 'hidden' }}>
      ...
      <div style={{ padding: '24px 32px 20px', display: 'flex', alignItems: 'center', justifyContent: 'space-between' }}>
        ...
        <button onClick={() => setCreating(true)} style={{
          ...
          color: 'var(--foreground)', border: 'none', fontSize: 14, fontWeight: 500,
      ...
      {creating && (
        <div style={{ margin: '0 32px 16px', padding: 20, background: T.surface, borderRadius: 18, border: `1px solid ${T.border}` }}>
          <input value={newPath} onChange={e => setNewPath(e.target.value)}
            ...
            style={{ width: '100%', height: 40, borderRadius: 50, border: `1px solid ${T.border}`, padding: '0 14px', fontSize: 14, color: T.text, fontFamily: T.font, outline: 'none', background: T.bg, marginBottom: 8 }} />
          ...
          <div style={{ display: 'flex', gap: 8, marginTop: 8 }}>
            <button onClick={handleCreate} disabled={saving} style={{
              height: 36, padding: '0 16px', borderRadius: 50, background: T.accent, color: 'var(--foreground)',
            ...
            <button onClick={() => { setCreating(false); setNewPath(''); setNewContent('') }} style={{
              ...
              border: `1px solid ${T.border}`, fontSize: 14, cursor: 'pointer', fontFamily: T.font
            }}>Cancel</button>
          </div>
        </div>
      )}
      ...
      <div style={{ flex: 1, overflowY: 'auto', padding: '0 32px 32px' }}>
        {loading ? (
          <div style={{ textAlign: 'center', padding: 40, color: T.textTertiary, fontSize: 14 }}>Loading memories...</div>
        ) : memories.length === 0 ? (
          <div style={{ textAlign: 'center', padding: 40, color: T.textTertiary, fontSize: 14 }}>No memories stored yet. Kiko will create them as you interact.</div>
        ) : (
              ...
              return (
                <div key={mem.id} style={{ background: T.surface, borderRadius: 18, border: `1px solid ${T.border}`, overflow: 'hidden', transition: 'border-color 0.15s' }}>
                  ...
                  <div onClick={() => { setExpanded(isExpanded ? null : mem.id); if (isEditing) setEditing(null) }}
                    ...
                    <span style={{ flex: 1, fontSize: 15, fontWeight: 500, color: T.text, fontFamily: T.font }}>{name}</span>
                    <span style={{ fontSize: 12, color: T.textTertiary, fontFamily: T.font }}>{lines} lines · {size}KB</span>
                    <span style={{ fontSize: 12, color: T.textTertiary, fontFamily: T.font, display: 'flex', alignItems: 'center', gap: 4 }}>
                  ...
                  {isExpanded && (
                    <div style={{ padding: '0 20px 16px', borderTop: `1px solid ${T.border}` }}>
                      {isEditing ? (
                        <div style={{ paddingTop: 12 }}>
                          ...
                          <div style={{ display: 'flex', gap: 8, marginTop: 8 }}>
                            <button onClick={() => handleSave(mem.id)} disabled={saving} style={{
                              height: 32, padding: '0 14px', borderRadius: 50, background: T.accent, color: 'var(--foreground)',
                        ...
                        <div style={{ paddingTop: 12 }}>
                          <pre style={{ fontSize: 14, color: T.textSecondary, fontFamily: 'monospace', lineHeight: 1.6, whiteSpace: 'pre-wrap', wordBreak: 'break-word', margin: 0 }}>
                            {mem.content}
                          </pre>
