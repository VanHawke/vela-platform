...
export default function Pipeline({ user }) {
  ...
  const stageAccent = {
    'To revisit': 'rgba(167,139,250,0.10)',
    'Contact made': 'rgba(167,139,250,0.3)',
  ...
  const stageTextColor = {
    'To revisit': 'rgba(167,139,250,0.35)',
    'Contact made': 'rgba(167,139,250,0.6)',
  ...
  const sectionTitle = { fontSize: 12, fontWeight: 300, color: 'rgba(167,139,250,0.35)', fontFamily: 'var(--font)', margin: '0 0 10px', textTransform: 'uppercase', letterSpacing: '0.06em' }
  const emptyText = { fontSize: 13, color: 'rgba(167,139,250,0.30)', fontFamily: 'var(--font)', fontStyle: 'italic', fontWeight: 300 }
  ...
  return (
    <div style={{ flex: 1, display: 'flex', flexDirection: 'column', overflow: 'hidden', paddingTop: 8 }}>
      ...
      <div style={{
        ...
        border: '0.5px solid rgba(167,139,250,0.50)', borderTop: '0.5px solid rgba(167,139,250,0.10)',
        boxShadow: '0 8px 32px rgba(0,0,0,0.35), inset 0 1px 0 rgba(167,139,250,0.06)',
        ...
        <div style={{ display: 'flex', alignItems: 'center', gap: 12 }}>
          <label style={{ display: 'flex', alignItems: 'center', gap: 6, cursor: 'pointer' }}>
            <input type="checkbox" checked={showClosed} onChange={e => setShowClosed(e.target.checked)} style={{ width: 14, height: 14, accentColor: 'rgba(167,139,250,0.12)' }} />
              ...
              return (
                <div key={stage.id}
                  ...
                  style={{
                    ...
                    borderRadius: 16, border: isOver ? '1px dashed rgba(167,139,250,0.3)' : '0.5px solid rgba(167,139,250,0.06)',
                  ...
                  <div style={{ padding: '12px 14px 10px', display: 'flex', alignItems: 'center', justifyContent: 'space-between' }}>
                    <div style={{ display: 'flex', alignItems: 'center', gap: 8 }}>
                      <span style={{ fontSize: 11, fontWeight: 400, color: stageTextColor[stage.id] || 'rgba(167,139,250,0.35)', fontFamily: 'var(--font)', textTransform: 'uppercase', letterSpacing: '0.06em' }}>{stage.label}</span>
                      <span style={{ fontSize: 10, fontWeight: 300, color: stageTextColor[stage.id] || 'rgba(167,139,250,0.25)', fontFamily: 'var(--font)', background: `${stageAccent[stage.id] || 'rgba(25,25,25,0.40)'}33`, borderRadius: 50, padding: '2px 6px' }}>{stageDeals.length}</span>
                    </div>
                  </div>
                  <div style={{ flex: 1, overflowY: 'auto', padding: '4px 8px 8px', display: 'flex', flexDirection: 'column', gap: 4 }}>
                    {loading ? (
                      ...
                      <p style={{ fontSize: 12, color: 'rgba(167,139,250,0.12)', textAlign: 'center', padding: '20px 0', fontFamily: 'var(--font)', fontWeight: 300 }}>No deals</p>
                    ) : stageDeals.map(deal => (
                      <div key={deal._id}
                        draggable
                        ...
                        onMouseEnter={e => { e.currentTarget.style.background = 'rgba(167,139,250,0.50)'; e.currentTarget.style.borderColor = 'rgba(167,139,250,0.10)'; e.currentTarget.style.borderLeftColor = stageAccent[stage.id] || 'rgba(167,139,250,0.10)' }}
                        onMouseLeave={e => { e.currentTarget.style.background = 'rgba(25,25,25,0.55)'; e.currentTarget.style.borderColor = 'rgba(167,139,250,0.50)'; e.currentTarget.style.borderLeftColor = stageAccent[stage.id] || 'rgba(167,139,250,0.10)' }}>
                        <div style={{ display: 'flex', alignItems: 'center', gap: 8 }}>
                        ...
                        {deal.contactName && (
                          <p style={{ fontSize: 11, color: 'rgba(167,139,250,0.30)', margin: '4px 0 0', fontFamily: 'var(--font)' }}>
                            {deal.contactName}{deal.industry ? ` · ${deal.industry}` : ''}
                          </p>
                        )}
                        {!deal.contactName && deal.industry && (
                          <p style={{ fontSize: 11, color: 'rgba(167,139,250,0.25)', margin: '4px 0 0', fontFamily: 'var(--font)' }}>{deal.industry}</p>
                        )}
                        {/* Pipeline badge — visible in All view */}
                        {pipelineFilter === 'All' && deal.pipeline && (
                          <div style={{ display: 'flex', alignItems: 'center', gap: 4, marginTop: 4 }}>
                            <span style={{ fontSize: 9, padding: '1px 6px', borderRadius: 4, background: 'rgba(167,139,250,0.08)', color: 'rgba(167,139,250,0.6)', fontWeight: 500, fontFamily: 'var(--font)' }}>{deal.pipeline}</span>
                          </div>
                        )}
        ...
        <div style={{ width: panelOpen ? 380 : 0, minWidth: panelOpen ? 380 : 0, transition: 'width 0.3s ease, min-width 0.3s ease, opacity 0.2s ease', opacity: panelOpen ? 1 : 0, overflow: 'hidden', marginLeft: panelOpen ? 16 : 0 }}>
          {selectedDeal && (
            <div style={{ width: 380, height: '100%', overflowY: 'auto', display: 'flex', flexDirection: 'column', gap: 14, paddingTop: 16 }}>
              <div style={{ background: 'rgba(25,25,25,0.40)', backdropFilter: 'blur(40px)', WebkitBackdropFilter: 'blur(40px)', borderRadius: 18, padding: '20px 20px 16px', border: '0.5px solid rgba(167,139,250,0.50)' }}>
                <div style={{ display: 'flex', alignItems: 'flex-start', justifyContent: 'space-between', marginBottom: 12 }}>
                  <div style={{ display: 'flex', alignItems: 'center', gap: 12 }}>
                    ...
                    <div>
                      ...
                      {dealCompany?.industry && <p style={{ fontSize: 13, color: 'rgba(167,139,250,0.45)', margin: '3px 0 0', fontFamily: 'var(--font)', fontWeight: 300 }}>{dealCompany.industry}{dealCompany.country ? ` · ${dealCompany.country}` : ''}</p>}
                    </div>
                  </div>
                  <button onClick={closePanel} style={{ background: 'rgba(25,25,25,0.40)', border: '0.5px solid rgba(167,139,250,0.50)', borderRadius: 50, width: 28, height: 28, display: 'flex', alignItems: 'center', justifyContent: 'center', cursor: 'pointer', color: 'rgba(167,139,250,0.35)', flexShrink: 0 }}><X style={{ width: 14, height: 14 }} /></button>
                </div>
                ...
                {dealCompany && (
                  <button onClick={() => nav(`/organisations?org=${dealCompany.id}`)} style={{ display: 'flex', alignItems: 'center', gap: 6, fontSize: 13, color: 'var(--accent)', background: 'rgba(167,139,250,0.06)', padding: '6px 12px', borderRadius: 50, border: '1.5px solid rgba(167,139,250,0.12)', cursor: 'pointer', fontFamily: 'var(--font)', fontWeight: 300 }}>
                    <ExternalLink style={{ width: 12, height: 12 }} /> View Organisation
                  </button>
                )}
              </div>
              <div style={{ background: 'rgba(25,25,25,0.40)', backdropFilter: 'blur(40px)', WebkitBackdropFilter: 'blur(40px)', borderRadius: 18, padding: '16px 20px', border: '0.5px solid rgba(167,139,250,0.50)' }}>
                  ...
                  <div style={{ display: 'flex', flexDirection: 'column', gap: 6 }}>
                    {dealCampaigns.map(camp => (
                      <div key={camp.name} style={{ padding: '8px 10px', background: 'rgba(59,130,246,0.06)', borderRadius: 50, border: '1.5px solid rgba(59,130,246,0.12)' }}>
                        ...
                        <p style={{ fontSize: 11, color: 'rgba(167,139,250,0.25)', margin: '2px 0 0', fontFamily: 'var(--font)', fontWeight: 300 }}>{camp.contacts} contact{camp.contacts !== 1 ? 's' : ''}</p>
                      </div>
                    ))}
                  </div>
                ...
                {activityType && (
                  <div style={{ display: 'flex', gap: 6 }}>
                    <input value={activityNote} onChange={e => setActivityNote(e.target.value)} placeholder={`Notes for ${activityType}...`}
                      ...
                      style={{ flex: 1, background: 'rgba(25,25,25,0.30)', border: '0.5px solid rgba(167,139,250,0.50)', borderRadius: 8, padding: '6px 10px', fontSize: 12, color: 'rgba(238,232,220,0.70)', fontFamily: 'var(--font)', outline: 'none' }} />
