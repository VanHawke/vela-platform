  ...
  const dropdown = open ? createPortal(
    <div
      ...
      style={{
        ...
        boxShadow: '0 8px 40px var(--border), 0 2px 8px var(--card)',
        overflow: 'hidden',
      }}
    >
...
export default function Pipeline({ user }) {
  ...
  const staleStyle = (d) => {
    ...
    const diff = Math.floor((new Date() - new Date(d)) / 86400000)
    if (diff > 60) return { color: '#ef4444' }; if (diff > 30) return { color: 'var(--primary)' }
  ...
  const stageAccent = {
    'To revisit': 'var(--accent)',
    'Contact made': 'var(--ring)',
  ...
  const sectionTitle = { fontSize: 12, fontWeight: 300, color: 'var(--ring)', fontFamily: 'var(--font)', margin: '0 0 10px', textTransform: 'uppercase', letterSpacing: '0.06em' }
  const emptyText = { fontSize: 13, color: 'var(--ring)', fontFamily: 'var(--font)', fontStyle: 'italic', fontWeight: 300 }
  ...
  return (
    <div style={{ flex: 1, display: 'flex', flexDirection: 'column', overflow: 'hidden', paddingTop: 8 }}>
      ...
      <div style={{
        ...
        background: 'var(--card)', backdropFilter: 'blur(30px) saturate(1.3)', WebkitBackdropFilter: 'blur(30px) saturate(1.3)',
        border: '0.5px solid var(--ring)', borderTop: '0.5px solid var(--accent)',
        boxShadow: '0 8px 32px var(--border), inset 0 1px 0 var(--accent)',
        ...
        <div style={{ display: 'flex', alignItems: 'center', gap: 12 }}>
          <label style={{ display: 'flex', alignItems: 'center', gap: 6, cursor: 'pointer' }}>
            <input type="checkbox" checked={showClosed} onChange={e => setShowClosed(e.target.checked)} style={{ width: 14, height: 14, accentColor: 'var(--accent)' }} />
              ...
              return (
                <div key={stage.id}
                  ...
                  style={{
                    ...
                    borderRadius: 16, border: isOver ? '1px dashed var(--ring)' : '0.5px solid var(--accent)',
                    background: isOver ? 'var(--card)' : 'var(--card)', backdropFilter: 'blur(20px) saturate(1.15)', WebkitBackdropFilter: 'blur(20px) saturate(1.15)',
                    boxShadow: '0 12px 40px var(--border), 0 1px 0 var(--card) inset',
                    transition: 'all 0.15s ease',
                  }}>
                  <div style={{ padding: '12px 14px 10px', display: 'flex', alignItems: 'center', justifyContent: 'space-between' }}>
                    <div style={{ display: 'flex', alignItems: 'center', gap: 8 }}>
                      <span style={{ fontSize: 11, fontWeight: 400, color: stageTextColor[stage.id] || 'var(--ring)', fontFamily: 'var(--font)', textTransform: 'uppercase', letterSpacing: '0.06em' }}>{stage.label}</span>
                      <span style={{ fontSize: 10, fontWeight: 300, color: stageTextColor[stage.id] || 'var(--ring)', fontFamily: 'var(--font)', background: `${stageAccent[stage.id] || 'var(--card)'}33`, borderRadius: 50, padding: '2px 6px' }}>{stageDeals.length}</span>
                    </div>
                  </div>
                  <div style={{ flex: 1, overflowY: 'auto', padding: '4px 8px 8px', display: 'flex', flexDirection: 'column', gap: 4 }}>
                    {loading ? (
                      [...Array(2)].map((_, i) => <div key={i} style={{ height: 70, background: 'var(--card)', borderRadius: 12 }} />)
                    ) : stageDeals.length === 0 ? (
                      <p style={{ fontSize: 12, color: 'var(--accent)', textAlign: 'center', padding: '20px 0', fontFamily: 'var(--font)', fontWeight: 300 }}>No deals</p>
                    ) : stageDeals.map(deal => (
                      <div key={deal._id}
                        draggable
                        ...
                        style={{ background: 'var(--card)', backdropFilter: 'blur(16px)', WebkitBackdropFilter: 'blur(16px)', borderRadius: 10, padding: '12px 14px', border: '0.5px solid var(--ring)', borderTop: '0.5px solid var(--accent)', borderLeft: `3px solid ${stageAccent[stage.id] || 'var(--accent)'}`, cursor: 'grab', transition: 'all 0.15s ease', position: 'relative', boxShadow: '0 4px 16px var(--border), inset 0 1px 0 var(--accent)' }}
                        onMouseEnter={e => { e.currentTarget.style.background = 'var(--ring)'; e.currentTarget.style.borderColor = 'var(--accent)'; e.currentTarget.style.borderLeftColor = stageAccent[stage.id] || 'var(--accent)' }}
                        onMouseLeave={e => { e.currentTarget.style.background = 'var(--card)'; e.currentTarget.style.borderColor = 'var(--ring)'; e.currentTarget.style.borderLeftColor = stageAccent[stage.id] || 'var(--accent)' }}>
                        <div style={{ display: 'flex', alignItems: 'center', gap: 8 }}>
                        ...
                        {deal.contactName && (
                          <p style={{ fontSize: 11, color: 'var(--ring)', margin: '4px 0 0', fontFamily: 'var(--font)' }}>
                            {deal.contactName}{deal.industry ? ` · ${deal.industry}` : ''}
                          </p>
                        )}
                        {!deal.contactName && deal.industry && (
                          <p style={{ fontSize: 11, color: 'var(--ring)', margin: '4px 0 0', fontFamily: 'var(--font)' }}>{deal.industry}</p>
                        )}
                        {/* Pipeline badge — visible in All view */}
                        {pipelineFilter === 'All' && deal.pipeline && (
                          <div style={{ display: 'flex', alignItems: 'center', gap: 4, marginTop: 4 }}>
                            <span style={{ fontSize: 9, padding: '1px 6px', borderRadius: 4, background: 'var(--accent)', color: 'var(--primary)', fontWeight: 500, fontFamily: 'var(--font)' }}>{deal.pipeline}</span>
                          </div>
                        )}
                        <div style={{ marginTop: 6 }}>
                          <span style={{ fontSize: 11, fontFamily: 'var(--font)', ...staleStyle(deal.lastActivity) }}>
                            {daysAgo(deal.lastActivity) ? (staleStyle(deal.lastActivity).color === '#ef4444' ? '⏱ ' : staleStyle(deal.lastActivity).color === 'var(--primary)' ? '⏱ ' : '✓ ') : ''}{daysAgo(deal.lastActivity) || 'No activity'}
                          </span>
                        </div>
                      </div>
                    ))}
        ...
        <div style={{ width: panelOpen ? 380 : 0, minWidth: panelOpen ? 380 : 0, transition: 'width 0.3s ease, min-width 0.3s ease, opacity 0.2s ease', opacity: panelOpen ? 1 : 0, overflow: 'hidden', marginLeft: panelOpen ? 16 : 0 }}>
          {selectedDeal && (
            <div style={{ width: 380, height: '100%', overflowY: 'auto', display: 'flex', flexDirection: 'column', gap: 14, paddingTop: 16 }}>
              <div style={{ background: 'var(--card)', backdropFilter: 'blur(40px)', WebkitBackdropFilter: 'blur(40px)', borderRadius: 18, padding: '20px 20px 16px', border: '0.5px solid var(--ring)' }}>
                <div style={{ display: 'flex', alignItems: 'flex-start', justifyContent: 'space-between', marginBottom: 12 }}>
                  <div style={{ display: 'flex', alignItems: 'center', gap: 12 }}>
                    {companyDomains[selectedDeal.company] ? (
                      <div style={{ width: 40, height: 40, borderRadius: 50, background: 'var(--card)', border: '0.5px solid var(--ring)', display: 'flex', alignItems: 'center', justifyContent: 'center', flexShrink: 0, overflow: 'hidden' }}>
                    ...
                    <div>
                      ...
                      {dealCompany?.industry && <p style={{ fontSize: 13, color: 'var(--ring)', margin: '3px 0 0', fontFamily: 'var(--font)', fontWeight: 300 }}>{dealCompany.industry}{dealCompany.country ? ` · ${dealCompany.country}` : ''}</p>}
                    </div>
                  </div>
                  <button onClick={closePanel} style={{ background: 'var(--card)', border: '0.5px solid var(--ring)', borderRadius: 50, width: 28, height: 28, display: 'flex', alignItems: 'center', justifyContent: 'center', cursor: 'pointer', color: 'var(--ring)', flexShrink: 0 }}><X style={{ width: 14, height: 14 }} /></button>
                </div>
                ...
                {dealCompany && (
                  <button onClick={() => nav(`/organisations?org=${dealCompany.id}`)} style={{ display: 'flex', alignItems: 'center', gap: 6, fontSize: 13, color: 'var(--accent)', background: 'var(--accent)', padding: '6px 12px', borderRadius: 50, border: '1.5px solid var(--accent)', cursor: 'pointer', fontFamily: 'var(--font)', fontWeight: 300 }}>
                    <ExternalLink style={{ width: 12, height: 12 }} /> View Organisation
                  </button>
                )}
              </div>
              <div style={{ background: 'var(--card)', backdropFilter: 'blur(40px)', WebkitBackdropFilter: 'blur(40px)', borderRadius: 18, padding: '16px 20px', border: '0.5px solid var(--ring)' }}>
                ...
                {loadingPanel ? (
                  <div style={{ display: 'flex', flexDirection: 'column', gap: 6 }}>{[...Array(2)].map((_, i) => <div key={i} style={{ height: 40, background: 'var(--card)', borderRadius: 50 }} />)}</div>
                      ...
                      <div key={ct.id} onClick={() => nav(`/contacts/${ct.id}`)}
                        ...
                        {ct.picture ? <img src={ct.picture} alt="" style={{ width: 28, height: 28, borderRadius: 50, objectFit: 'cover' }} /> : (
                          <div style={{ width: 28, height: 28, borderRadius: 50, background: 'var(--card)', display: 'flex', alignItems: 'center', justifyContent: 'center', flexShrink: 0 }}>
                            <span style={{ fontSize: 12, fontWeight: 400, color: 'var(--ring)', fontFamily: 'var(--font)' }}>{(ct.firstName || '?')[0]?.toUpperCase()}</span>
                          </div>
                        )}
                  ...
                  <div style={{ display: 'flex', flexDirection: 'column', gap: 6 }}>
                    {dealCampaigns.map(camp => (
                      <div key={camp.name} style={{ padding: '8px 10px', background: 'rgba(59,130,246,0.06)', borderRadius: 50, border: '1.5px solid rgba(59,130,246,0.12)' }}>
                        ...
                        <p style={{ fontSize: 11, color: 'var(--ring)', margin: '2px 0 0', fontFamily: 'var(--font)', fontWeight: 300 }}>{camp.contacts} contact{camp.contacts !== 1 ? 's' : ''}</p>
                      </div>
                    ))}
                  </div>
                ...
                <div style={{ display: 'flex', gap: 6, marginBottom: 8, flexWrap: 'wrap' }}>
                  {['Call', 'Meeting', 'Email Sent', 'LinkedIn', 'Note'].map(t => (
                    <button key={t} onClick={() => activityType === t ? setActivityType(null) : setActivityType(t)}
                      style={{ padding: '4px 10px', borderRadius: 8, fontSize: 11, fontWeight: 400, fontFamily: 'var(--font)', cursor: 'pointer', border: `1px solid ${activityType === t ? 'var(--ring)' : 'var(--accent)'}`, background: activityType === t ? 'var(--accent)' : 'var(--card)', color: activityType === t ? 'var(--primary)' : 'var(--ring)', transition: 'all 0.15s' }}>{t}</button>
                  ))}
                </div>
                {activityType && (
                  <div style={{ display: 'flex', gap: 6 }}>
                    <input value={activityNote} onChange={e => setActivityNote(e.target.value)} placeholder={`Notes for ${activityType}...`}
                      ...
                      style={{ flex: 1, background: 'var(--card)', border: '0.5px solid var(--ring)', borderRadius: 8, padding: '6px 10px', fontSize: 12, color: 'rgba(238,232,220,0.70)', fontFamily: 'var(--font)', outline: 'none' }} />
