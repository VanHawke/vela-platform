  ...
  return (
    <div style={{ flex: 1, display: 'flex', flexDirection: 'column', overflow: 'hidden', paddingTop: 8 }}>
      ...
      <div style={{
        ...
        boxShadow: '0 8px 32px rgba(0,0,0,0.35), inset 0 1px 0 var(--accent)',
                ...
                <div key={stage.id}
                  ...
                  <div style={{ padding: '12px 14px 10px', display: 'flex', alignItems: 'center', justifyContent: 'space-between' }}>
                    <div style={{ display: 'flex', alignItems: 'center', gap: 8 }}>
                      ...
                      <span style={{ fontSize: 10, fontWeight: 300, color: stageTextColor[stage.id] || 'var(--ring)', fontFamily: 'var(--font)', background: `${stageAccent[stage.id] || 'rgba(25,25,25,0.40)'}33`, borderRadius: 50, padding: '2px 6px' }}>{stageDeals.length}</span>
                    </div>
                  </div>
                  <div style={{ flex: 1, overflowY: 'auto', padding: '4px 8px 8px', display: 'flex', flexDirection: 'column', gap: 4 }}>
                    {loading ? (
                      ...
                      <div key={deal._id}
                        ...
                        style={{ background: 'rgba(25,25,25,0.40)', backdropFilter: 'blur(16px)', WebkitBackdropFilter: 'blur(16px)', borderRadius: 10, padding: '12px 14px', border: '0.5px solid var(--ring)', borderTop: '0.5px solid var(--accent)', borderLeft: `3px solid ${stageAccent[stage.id] || 'var(--accent)'}`, cursor: 'grab', transition: 'all 0.15s ease', position: 'relative', boxShadow: '0 4px 16px rgba(0,0,0,0.25), inset 0 1px 0 var(--accent)' }}
                        ...
                        onMouseLeave={e => { e.currentTarget.style.background = 'rgba(25,25,25,0.55)'; e.currentTarget.style.borderColor = 'var(--ring)'; e.currentTarget.style.borderLeftColor = stageAccent[stage.id] || 'var(--accent)' }}>
                        <div style={{ display: 'flex', alignItems: 'center', gap: 8 }}>
        ...
        <div style={{ width: panelOpen ? 380 : 0, minWidth: panelOpen ? 380 : 0, transition: 'width 0.3s ease, min-width 0.3s ease, opacity 0.2s ease', opacity: panelOpen ? 1 : 0, overflow: 'hidden', marginLeft: panelOpen ? 16 : 0 }}>
          {selectedDeal && (
            <div style={{ width: 380, height: '100%', overflowY: 'auto', display: 'flex', flexDirection: 'column', gap: 14, paddingTop: 16 }}>
              <div style={{ background: 'rgba(25,25,25,0.40)', backdropFilter: 'blur(40px)', WebkitBackdropFilter: 'blur(40px)', borderRadius: 18, padding: '20px 20px 16px', border: '0.5px solid var(--ring)' }}>
                <div style={{ display: 'flex', alignItems: 'flex-start', justifyContent: 'space-between', marginBottom: 12 }}>
                  <div style={{ display: 'flex', alignItems: 'center', gap: 12 }}>
                    {companyDomains[selectedDeal.company] ? (
                      <div style={{ width: 40, height: 40, borderRadius: 50, background: 'rgba(25,25,25,0.40)', border: '0.5px solid var(--ring)', display: 'flex', alignItems: 'center', justifyContent: 'center', flexShrink: 0, overflow: 'hidden' }}>
                  ...
                  <button onClick={closePanel} style={{ background: 'rgba(25,25,25,0.40)', border: '0.5px solid var(--ring)', borderRadius: 50, width: 28, height: 28, display: 'flex', alignItems: 'center', justifyContent: 'center', cursor: 'pointer', color: 'var(--ring)', flexShrink: 0 }}><X style={{ width: 14, height: 14 }} /></button>
                </div>
              ...
              <div style={{ background: 'rgba(25,25,25,0.40)', backdropFilter: 'blur(40px)', WebkitBackdropFilter: 'blur(40px)', borderRadius: 18, padding: '16px 20px', border: '0.5px solid var(--ring)' }}>
                ...
                <div style={{ display: 'flex', gap: 6, marginBottom: 8, flexWrap: 'wrap' }}>
                  {['Call', 'Meeting', 'Email Sent', 'LinkedIn', 'Note'].map(t => (
                    <button key={t} onClick={() => activityType === t ? setActivityType(null) : setActivityType(t)}
                      style={{ padding: '4px 10px', borderRadius: 8, fontSize: 11, fontWeight: 400, fontFamily: 'var(--font)', cursor: 'pointer', border: `1px solid ${activityType === t ? 'var(--ring)' : 'var(--accent)'}`, background: activityType === t ? 'var(--accent)' : 'rgba(25,25,25,0.30)', color: activityType === t ? 'var(--primary)' : 'var(--ring)', transition: 'all 0.15s' }}>{t}</button>
                  ))}
                </div>
                {activityType && (
                  <div style={{ display: 'flex', gap: 6 }}>
                    <input value={activityNote} onChange={e => setActivityNote(e.target.value)} placeholder={`Notes for ${activityType}...`}
                      ...
                      style={{ flex: 1, background: 'rgba(25,25,25,0.30)', border: '0.5px solid var(--ring)', borderRadius: 8, padding: '6px 10px', fontSize: 12, color: 'rgba(238,232,220,0.70)', fontFamily: 'var(--font)', outline: 'none' }} />
