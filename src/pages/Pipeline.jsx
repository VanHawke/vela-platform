  ...
  const dropdown = open ? createPortal(
    <div
      ...
      style={{
        ...
        boxShadow: '0 8px 40px rgba(0,0,0,0.13), 0 2px 8px rgba(238,238,238,0.04)',
        overflow: 'hidden',
      }}
    >
          ...
          <div style={{ display: 'flex', gap: 6, marginTop: 8 }}>
            <button
              ...
              style={{
                ...
                fontSize: 14, fontWeight: 500, color: newName.trim() ? 'rgba(238,238,238,0.9)' : 'var(--text-tertiary)',
...
export default function Pipeline({ user }) {
  ...
  const stageAccent = {
    'To revisit': 'rgba(238,238,238,0.1)',
    'Contact made': 'rgba(255,224,194,0.3)',
  ...
  const stageTextColor = {
    'To revisit': 'rgba(238,238,238,0.3)',
    'Contact made': 'rgba(255,224,194,0.6)',
  ...
  const sectionTitle = { fontSize: 12, fontWeight: 300, color: 'rgba(238,238,238,0.3)', fontFamily: 'var(--font)', margin: '0 0 10px', textTransform: 'uppercase', letterSpacing: '0.06em' }
  const emptyText = { fontSize: 13, color: 'rgba(238,238,238,0.25)', fontFamily: 'var(--font)', fontStyle: 'italic', fontWeight: 300 }
  ...
  return (
    <div style={{ flex: 1, display: 'flex', flexDirection: 'column', overflow: 'hidden', paddingTop: 8 }}>
      ...
      <div style={{
        ...
        background: 'rgba(238,238,238,0.025)', backdropFilter: 'blur(30px) saturate(1.3)', WebkitBackdropFilter: 'blur(30px) saturate(1.3)',
        border: '0.5px solid rgba(238,238,238,0.08)', borderTop: '0.5px solid rgba(238,238,238,0.12)',
        boxShadow: '0 8px 32px rgba(0,0,0,0.35), 0 1px 0 rgba(238,238,238,0.03) inset',
        ...
        <div style={{ display: 'flex', alignItems: 'center', gap: 12 }}>
          <label style={{ display: 'flex', alignItems: 'center', gap: 6, cursor: 'pointer' }}>
            <input type="checkbox" checked={showClosed} onChange={e => setShowClosed(e.target.checked)} style={{ width: 14, height: 14, accentColor: 'rgba(238,238,238,0.12)' }} />
              ...
              return (
                <div key={stage.id}
                  ...
                  style={{
                    ...
                    borderRadius: 16, border: isOver ? '1px dashed rgba(255,224,194,0.3)' : '0.5px solid rgba(238,238,238,0.06)',
                    background: isOver ? 'rgba(238,238,238,0.04)' : 'rgba(238,238,238,0.02)', backdropFilter: 'blur(20px) saturate(1.15)', WebkitBackdropFilter: 'blur(20px) saturate(1.15)',
                    boxShadow: '0 12px 40px rgba(0,0,0,0.35), 0 1px 0 rgba(238,238,238,0.02) inset',
                    transition: 'all 0.15s ease',
                  }}>
                  <div style={{ padding: '12px 14px 10px', display: 'flex', alignItems: 'center', justifyContent: 'space-between' }}>
                    <div style={{ display: 'flex', alignItems: 'center', gap: 8 }}>
                      <span style={{ fontSize: 11, fontWeight: 400, color: stageTextColor[stage.id] || 'rgba(238,238,238,0.3)', fontFamily: 'var(--font)', textTransform: 'uppercase', letterSpacing: '0.06em' }}>{stage.label}</span>
                      <span style={{ fontSize: 10, fontWeight: 300, color: stageTextColor[stage.id] || 'rgba(238,238,238,0.2)', fontFamily: 'var(--font)', background: `${stageAccent[stage.id] || 'rgba(238,238,238,0.04)'}33`, borderRadius: 50, padding: '2px 6px' }}>{stageDeals.length}</span>
                    </div>
                  </div>
                  <div style={{ flex: 1, overflowY: 'auto', padding: '4px 8px 8px', display: 'flex', flexDirection: 'column', gap: 4 }}>
                    {loading ? (
                      [...Array(2)].map((_, i) => <div key={i} style={{ height: 70, background: 'rgba(238,238,238,0.02)', borderRadius: 12 }} />)
                    ) : stageDeals.length === 0 ? (
                      <p style={{ fontSize: 12, color: 'rgba(238,238,238,0.12)', textAlign: 'center', padding: '20px 0', fontFamily: 'var(--font)', fontWeight: 300 }}>No deals</p>
                    ) : stageDeals.map(deal => (
                      <div key={deal._id}
                        draggable
                        ...
                        style={{ background: 'rgba(238,238,238,0.04)', backdropFilter: 'blur(16px)', WebkitBackdropFilter: 'blur(16px)', borderRadius: 10, padding: '12px 14px', border: '0.5px solid rgba(238,238,238,0.08)', borderTop: '0.5px solid rgba(238,238,238,0.12)', borderLeft: `3px solid ${stageAccent[stage.id] || 'rgba(238,238,238,0.1)'}`, cursor: 'grab', transition: 'all 0.15s ease', position: 'relative', boxShadow: '0 4px 16px rgba(0,0,0,0.25), 0 1px 0 rgba(238,238,238,0.04) inset' }}
                        onMouseEnter={e => { e.currentTarget.style.background = 'rgba(238,238,238,0.05)'; e.currentTarget.style.borderColor = 'rgba(238,238,238,0.1)'; e.currentTarget.style.borderLeftColor = stageAccent[stage.id] || 'rgba(238,238,238,0.1)' }}
                        onMouseLeave={e => { e.currentTarget.style.background = 'rgba(238,238,238,0.025)'; e.currentTarget.style.borderColor = 'rgba(238,238,238,0.05)'; e.currentTarget.style.borderLeftColor = stageAccent[stage.id] || 'rgba(238,238,238,0.1)' }}>
                        <div style={{ display: 'flex', alignItems: 'center', gap: 8 }}>
                          ...
                          <p style={{ fontSize: 13, fontWeight: 400, color: 'rgba(238,238,238,0.82)', margin: 0, fontFamily: 'var(--font)', lineHeight: 1.3, overflow: 'hidden', textOverflow: 'ellipsis', whiteSpace: 'nowrap' }}>
                            {deal.company || deal.title}
                          </p>
                        </div>
                        {deal.contactName && (
                          <p style={{ fontSize: 11, color: 'rgba(238,238,238,0.25)', margin: '4px 0 0', fontFamily: 'var(--font)' }}>
                            {deal.contactName}{deal.industry ? ` · ${deal.industry}` : ''}
                          </p>
                        )}
                        {!deal.contactName && deal.industry && (
                          <p style={{ fontSize: 11, color: 'rgba(238,238,238,0.2)', margin: '4px 0 0', fontFamily: 'var(--font)' }}>{deal.industry}</p>
                        )}
                        {/* Pipeline badge — visible in All view */}
                        {pipelineFilter === 'All' && deal.pipeline && (
                          <div style={{ display: 'flex', alignItems: 'center', gap: 4, marginTop: 4 }}>
                            <span style={{ fontSize: 9, padding: '1px 6px', borderRadius: 4, background: 'rgba(255,224,194,0.08)', color: 'rgba(255,224,194,0.6)', fontWeight: 500, fontFamily: 'var(--font)' }}>{deal.pipeline}</span>
                          </div>
                        )}
        ...
        <div style={{ width: panelOpen ? 380 : 0, minWidth: panelOpen ? 380 : 0, transition: 'width 0.3s ease, min-width 0.3s ease, opacity 0.2s ease', opacity: panelOpen ? 1 : 0, overflow: 'hidden', marginLeft: panelOpen ? 16 : 0 }}>
          {selectedDeal && (
            <div style={{ width: 380, height: '100%', overflowY: 'auto', display: 'flex', flexDirection: 'column', gap: 14, paddingTop: 16 }}>
              <div style={{ background: 'rgba(238,238,238,0.04)', backdropFilter: 'blur(40px)', WebkitBackdropFilter: 'blur(40px)', borderRadius: 18, padding: '20px 20px 16px', border: '0.5px solid rgba(238,238,238,0.1)' }}>
                <div style={{ display: 'flex', alignItems: 'flex-start', justifyContent: 'space-between', marginBottom: 12 }}>
                  <div style={{ display: 'flex', alignItems: 'center', gap: 12 }}>
                    {companyDomains[selectedDeal.company] ? (
                      <div style={{ width: 40, height: 40, borderRadius: 50, background: 'rgba(238,238,238,0.04)', border: '0.5px solid rgba(238,238,238,0.1)', display: 'flex', alignItems: 'center', justifyContent: 'center', flexShrink: 0, overflow: 'hidden' }}>
                    ...
                    <div>
                      <h2 style={{ fontSize: 17, fontWeight: 400, color: 'rgba(238,238,238,0.9)', margin: 0, fontFamily: 'var(--font)' }}>{selectedDeal.company || selectedDeal.title}</h2>
                      {dealCompany?.industry && <p style={{ fontSize: 13, color: 'rgba(238,238,238,0.4)', margin: '3px 0 0', fontFamily: 'var(--font)', fontWeight: 300 }}>{dealCompany.industry}{dealCompany.country ? ` · ${dealCompany.country}` : ''}</p>}
                    </div>
                  </div>
                  <button onClick={closePanel} style={{ background: 'rgba(238,238,238,0.04)', border: '0.5px solid rgba(238,238,238,0.1)', borderRadius: 50, width: 28, height: 28, display: 'flex', alignItems: 'center', justifyContent: 'center', cursor: 'pointer', color: 'rgba(238,238,238,0.3)', flexShrink: 0 }}><X style={{ width: 14, height: 14 }} /></button>
                </div>
                ...
                {dealCompany && (
                  <button onClick={() => nav(`/organisations?org=${dealCompany.id}`)} style={{ display: 'flex', alignItems: 'center', gap: 6, fontSize: 13, color: 'var(--accent)', background: 'rgba(255,224,194,0.06)', padding: '6px 12px', borderRadius: 50, border: '1.5px solid rgba(255,224,194,0.12)', cursor: 'pointer', fontFamily: 'var(--font)', fontWeight: 300 }}>
                    <ExternalLink style={{ width: 12, height: 12 }} /> View Organisation
                  </button>
                )}
              </div>
              <div style={{ background: 'rgba(238,238,238,0.04)', backdropFilter: 'blur(40px)', WebkitBackdropFilter: 'blur(40px)', borderRadius: 18, padding: '16px 20px', border: '0.5px solid rgba(238,238,238,0.1)' }}>
                ...
                {loadingPanel ? (
                  <div style={{ display: 'flex', flexDirection: 'column', gap: 6 }}>{[...Array(2)].map((_, i) => <div key={i} style={{ height: 40, background: 'rgba(238,238,238,0.02)', borderRadius: 50 }} />)}</div>
                  ...
                  <div style={{ display: 'flex', flexDirection: 'column', gap: 4 }}>
                    {dealContacts.map((ct, i) => (
                      <div key={ct.id} onClick={() => nav(`/contacts/${ct.id}`)}
                        ...
                        onMouseEnter={e => e.currentTarget.style.background = 'rgba(238,238,238,0.04)'}
                        ...
                        {ct.picture ? <img src={ct.picture} alt="" style={{ width: 28, height: 28, borderRadius: 50, objectFit: 'cover' }} /> : (
                          <div style={{ width: 28, height: 28, borderRadius: 50, background: 'rgba(238,238,238,0.04)', display: 'flex', alignItems: 'center', justifyContent: 'center', flexShrink: 0 }}>
                            <span style={{ fontSize: 12, fontWeight: 400, color: 'rgba(238,238,238,0.4)', fontFamily: 'var(--font)' }}>{(ct.firstName || '?')[0]?.toUpperCase()}</span>
                          </div>
                        )}
                        <div style={{ minWidth: 0, flex: 1 }}>
                          <p style={{ fontSize: 13, fontWeight: 400, color: 'rgba(238,238,238,0.8)', margin: 0, fontFamily: 'var(--font)' }}>{[ct.firstName, ct.lastName].filter(Boolean).join(' ')}{i === 0 ? ' (Primary)' : ''}</p>
                  ...
                  <div style={{ display: 'flex', flexDirection: 'column', gap: 6 }}>
                    {dealCampaigns.map(camp => (
                      <div key={camp.name} style={{ padding: '8px 10px', background: 'rgba(59,130,246,0.06)', borderRadius: 50, border: '1.5px solid rgba(59,130,246,0.12)' }}>
                        <p style={{ fontSize: 14, fontWeight: 400, color: 'rgba(238,238,238,0.8)', margin: 0, fontFamily: 'var(--font)' }}>{camp.name}</p>
                        <p style={{ fontSize: 11, color: 'rgba(238,238,238,0.2)', margin: '2px 0 0', fontFamily: 'var(--font)', fontWeight: 300 }}>{camp.contacts} contact{camp.contacts !== 1 ? 's' : ''}</p>
                      </div>
                    ))}
                  </div>
                    ...
                    {dealTasks.map(task => {
                      ...
                      return (
                        <div key={task.id} onClick={() => nav('/command-centre')} style={{ padding: '8px 10px', background: isOverdue ? 'rgba(255,59,48,0.04)' : 'rgba(6,214,160,0.04)', borderRadius: 12, border: `1.5px solid ${isOverdue ? 'rgba(255,59,48,0.1)' : 'rgba(6,214,160,0.1)'}`, cursor: 'pointer', transition: 'all 0.15s' }}
                          ...
                          <p style={{ fontSize: 13, fontWeight: 400, color: 'rgba(238,238,238,0.7)', margin: '3px 0 0', fontFamily: 'var(--font)' }}>{d.notes || d.contact || 'View in Command Centre →'}</p>
                        </div>
                      )
                    })}
                  </div>
                ...
                <div style={{ display: 'flex', gap: 6, marginBottom: 8, flexWrap: 'wrap' }}>
                  {['Call', 'Meeting', 'Email Sent', 'LinkedIn', 'Note'].map(t => (
                    <button key={t} onClick={() => activityType === t ? setActivityType(null) : setActivityType(t)}
                      style={{ padding: '4px 10px', borderRadius: 8, fontSize: 11, fontWeight: 400, fontFamily: 'var(--font)', cursor: 'pointer', border: `1px solid ${activityType === t ? 'rgba(255,224,194,0.3)' : 'rgba(238,238,238,0.06)'}`, background: activityType === t ? 'rgba(255,224,194,0.08)' : 'rgba(238,238,238,0.02)', color: activityType === t ? 'rgba(255,224,194,0.8)' : 'rgba(238,238,238,0.35)', transition: 'all 0.15s' }}>{t}</button>
                  ))}
                </div>
                {activityType && (
                  <div style={{ display: 'flex', gap: 6 }}>
                    <input value={activityNote} onChange={e => setActivityNote(e.target.value)} placeholder={`Notes for ${activityType}...`}
                      ...
                      style={{ flex: 1, background: 'rgba(238,238,238,0.02)', border: '0.5px solid rgba(238,238,238,0.06)', borderRadius: 8, padding: '6px 10px', fontSize: 12, color: 'rgba(238,238,238,0.7)', fontFamily: 'var(--font)', outline: 'none' }} />
