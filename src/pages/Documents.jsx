...
const T = {
  ...
  border: 'var(--accent)', borderHover: 'rgba(238,232,220,0.70)', borderSubtle: 'var(--card)',
  text: 'rgba(238,232,220,0.95)', textSecondary: 'var(--ring)', textTertiary: 'var(--muted-foreground)',
  accent: 'var(--accent)', accentSoft: 'var(--card)',
...
export default function Documents({ user }) {
  ...
  const cardStyle = { background: 'var(--card)', backdropFilter: 'blur(40px) saturate(1.6)', WebkitBackdropFilter: 'blur(40px) saturate(1.6)', borderRadius: 16, border: '0.5px solid var(--ring)', boxShadow: 'inset 0 1px 0 var(--ring), 0 4px 24px var(--border)', padding: 16, transition: 'all 0.2s' }
  ...
  return (
    <div
      ...
      {dragOver && (
        <div style={{
          position: 'absolute', inset: 0, zIndex: 50, background: 'var(--card)',
      ...
      <div style={{ padding: '8px 24px 12px', display: 'flex', gap: 8, alignItems: 'center', flexWrap: 'wrap' }}>
        ...
        {/* Category pills */}
        <div style={{ display: 'flex', gap: 3, background: 'var(--card)', borderRadius: 50, padding: 3 }}>
          {CATEGORIES.map(c => (
            ...
            {filtered.map(doc => {
              ...
              return (
                <div key={doc.id} style={{ ...cardStyle, cursor: 'pointer' }} onClick={() => setExpanded(isExpanded ? null : doc.id)}>
                      ...
                      <div style={{ display: 'flex', alignItems: 'center', gap: 5, marginTop: 3, flexWrap: 'wrap' }}>
                        ...
                        {editingTeam === doc.id ? (
                          <select autoFocus value={doc.linked_team || ''} onChange={e => reassignTeam(doc.id, e.target.value)} onBlur={() => setEditingTeam(null)}
                            ...
                            style={{ fontSize: 11, padding: '1px 4px', borderRadius: 6, border: `1px solid ${T.blue}`, background: 'var(--card)', color: T.text, outline: 'none', fontFamily: T.font }}>
                  ...
                  {isExpanded && (
                    <div style={{ marginTop: 12, padding: 14, borderRadius: 50, background: 'rgba(0,0,0,0.02)', border: `1.5px solid var(--card)` }}>
                      ...
                      <div style={{ marginTop: 10, paddingTop: 10, borderTop: `1.5px solid var(--card)`, display: 'flex', gap: 12, fontSize: 11, color: T.textTertiary }}>
