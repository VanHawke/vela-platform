...
Be direct. Use web search for current company intelligence if needed.`
  ...
  const urgencyColor = (u) => u >= 3 ? 'rgba(255,59,48,0.6)' : u >= 2 ? 'rgba(245,158,11,0.5)' : u >= 1 ? 'rgba(255,224,194,0.4)' : 'rgba(238,238,238,0.1)'
  const card = { background: 'rgba(238,238,238,0.035)', backdropFilter: 'blur(24px) saturate(1.2)', WebkitBackdropFilter: 'blur(24px) saturate(1.2)', border: '0.5px solid rgba(238,238,238,0.1)', borderRadius: 14, padding: 16, boxShadow: '0 8px 32px rgba(0,0,0,0.3), 0 1px 0 rgba(238,238,238,0.05) inset' }
  ...
  return (
    <div style={{ display: 'flex', height: 'calc(100vh - 56px)', fontFamily: T.font }}>
      ...
      <div style={{ flex: 1, display: 'flex', flexDirection: 'column', overflow: 'hidden', minWidth: 0 }}>
        ...
        <div style={{ padding: '16px 20px 0' }}>
          <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', marginBottom: 12 }}>
            ...
            <button onClick={loadData} style={{ padding: '6px 12px', borderRadius: 8, background: 'rgba(238,238,238,0.04)', border: '0.5px solid rgba(238,238,238,0.06)', color: T.textTertiary, fontSize: 12, cursor: 'pointer', fontFamily: T.font, display: 'flex', alignItems: 'center', gap: 4 }}><RefreshCw size={10} /> Refresh</button>
          </div>
          ...
          <div style={{ display: 'flex', gap: 8, marginBottom: 14 }}>
            <div onClick={() => navigate('/pipeline')} style={{ ...card, flex: 1, display: 'flex', alignItems: 'center', gap: 10, padding: '10px 14px', cursor: 'pointer', transition: 'all 0.15s' }}
              onMouseOver={e => { e.currentTarget.style.borderColor = 'rgba(255,224,194,0.2)' }}
              onMouseOut={e => { e.currentTarget.style.borderColor = 'rgba(238,238,238,0.06)' }}>
              <Target size={14} style={{ color: 'rgba(255,224,194,0.5)', flexShrink: 0 }} />
              <div>
                <div style={{ fontSize: 17, fontWeight: 300, color: 'rgba(255,224,194,0.7)' }}>{deals.length}</div>
                ...
                <div style={{ display: 'flex', gap: 4, marginBottom: 2 }}>
                  {['F1', 'Formula E', 'MotoGP', 'WEC'].map(s => (
                    <button key={s} onClick={e => { e.stopPropagation(); setRaceSeries(s); setNextRace(allNextRaces[s] || null) }}
                      style={{ padding: '2px 8px', borderRadius: 50, border: raceSeries === s ? '1px solid rgba(0,212,170,0.3)' : '1px solid rgba(238,238,238,0.06)', background: raceSeries === s ? 'rgba(0,212,170,0.08)' : 'transparent', color: raceSeries === s ? 'rgba(0,212,170,0.7)' : 'rgba(238,238,238,0.25)', fontSize: 9, cursor: 'pointer', fontFamily: T.font, fontWeight: 400, transition: 'all 0.15s' }}>{s}</button>
                  ))}
                </div>
                <div style={{ display: 'flex', alignItems: 'center', gap: 10 }}>
                  <Calendar size={14} style={{ color: daysToRace <= 14 ? 'rgba(0,212,170,0.5)' : 'rgba(238,238,238,0.2)', flexShrink: 0 }} />
                  <div>
                    <div style={{ fontSize: 17, fontWeight: 300, color: daysToRace <= 14 ? 'rgba(0,212,170,0.6)' : 'rgba(238,238,238,0.4)' }}>{daysToRace}d</div>
        ...
        <div style={{ flex: 1, overflowY: 'auto', padding: '0 20px 20px' }}>
          ...
          {tasks.length > 0 && (<>
            ...
            {tasks.slice(0, 8).map(task => {
              ...
              return (
                <div key={task.id} style={{ display: 'flex', gap: 10, alignItems: 'flex-start', padding: '9px 12px', borderRadius: 10, marginBottom: 4, background: 'rgba(238,238,238,0.015)', border: '1px solid rgba(238,238,238,0.03)', cursor: 'pointer', transition: 'all 0.15s' }}
                  ...
                  <button onClick={(e) => { e.stopPropagation(); toggleTask(task) }} style={{ background: 'none', border: 'none', cursor: 'pointer', padding: 0, marginTop: 2, color: 'rgba(238,238,238,0.2)' }}>
                    <Square size={14} />
                  </button>
                  ...
                  <ChevronRight size={12} style={{ color: 'rgba(238,238,238,0.08)', flexShrink: 0, marginTop: 8 }} />
                </div>
              )
            })}
            <div style={{ height: 16 }} />
          ...
          {priorityActions.slice(0, 30).map((action, i) => {
            ...
            return (
              <div key={action.id} onClick={() => getKikoRec(action)} style={{
                ...
                background: isSelected ? 'rgba(255,224,194,0.03)' : 'rgba(238,238,238,0.015)',
                border: `1px solid ${isSelected ? 'rgba(255,224,194,0.2)' : 'rgba(238,238,238,0.03)'}`,
              }}
                onMouseEnter={e => { if (!isSelected) { e.currentTarget.style.background = 'rgba(238,238,238,0.03)'; e.currentTarget.style.borderColor = 'rgba(238,238,238,0.06)' }}}
                onMouseLeave={e => { if (!isSelected) { e.currentTarget.style.background = 'rgba(238,238,238,0.015)'; e.currentTarget.style.borderColor = 'rgba(238,238,238,0.03)' }}}
              >
                {/* Rank */}
                <span style={{ fontSize: 11, color: i < 3 ? 'rgba(255,224,194,0.6)' : T.textTertiary, fontWeight: 500, width: 16, textAlign: 'center', flexShrink: 0, marginTop: 3 }}>{i + 1}</span>
                {/* Company logo */}
                ...
                <div style={{ flex: 1, minWidth: 0 }}>
                  ...
                  <div style={{ display: 'flex', gap: 10, alignItems: 'center' }}>
                    ...
                    <span style={{ fontSize: 11, color: 'rgba(255,224,194,0.3)', fontWeight: 300 }}>{action.prob}%</span>
                  </div>
                </div>
                <ChevronRight size={12} style={{ color: 'rgba(238,238,238,0.08)', flexShrink: 0, marginTop: 8 }} />
              </div>
      ...
      <div style={{ flex: 1, borderLeft: '1px solid rgba(238,238,238,0.04)', display: 'flex', flexDirection: 'column', background: 'rgba(238,238,238,0.01)', flexShrink: 0, minWidth: 0 }}>
        <div style={{ padding: '14px 16px', borderBottom: '1px solid rgba(238,238,238,0.04)', display: 'flex', alignItems: 'center', gap: 8 }}>
          <div style={{ width: 40, height: 12, overflow: 'hidden' }}><KikoWaveform width={40} height={12} mini /></div>
          <span style={{ fontSize: 12, fontWeight: 500, color: 'rgba(255,224,194,0.6)', letterSpacing: '0.04em' }}>Kiko Intelligence</span>
        </div>

        <div style={{ flex: 1, overflowY: 'auto', padding: 16 }}>
          {!selectedAction && !kikoLoading && (
            <div style={{ textAlign: 'center', padding: '60px 20px' }}>
              <Target size={20} style={{ color: 'rgba(238,238,238,0.08)', margin: '0 auto 10px', display: 'block' }} />
          ...
          {kikoLoading && (
            <div style={{ display: 'flex', flexDirection: 'column', alignItems: 'center', gap: 12, padding: '40px 0' }}>
              ...
              <span style={{ fontSize: 13, color: 'rgba(255,224,194,0.6)', fontWeight: 400 }}>Analysing deal...</span>
            </div>
          )}
          ...
          {kikoRec && selectedAction && (
            <div>
              {/* Context header */}
              <div style={{ marginBottom: 12, padding: '10px 12px', borderRadius: 8, background: 'rgba(238,238,238,0.02)', border: '1px solid rgba(238,238,238,0.04)' }}>
                ...
                <div style={{ fontSize: 14, color: 'rgba(238,238,238,0.7)', fontWeight: 400 }}>
        ...
        {selectedAction && kikoRec && !kikoLoading && (
          <div style={{ padding: '10px 16px', borderTop: '1px solid rgba(238,238,238,0.04)', display: 'flex', gap: 6, flexWrap: 'wrap' }}>
            <button onClick={() => getKikoRec(selectedAction)} style={{ padding: '6px 12px', borderRadius: 8, fontSize: 12, fontWeight: 400, border: '1px solid rgba(255,224,194,0.15)', background: 'rgba(255,224,194,0.04)', color: 'rgba(255,224,194,0.7)', cursor: 'pointer', fontFamily: T.font, display: 'flex', alignItems: 'center', gap: 4 }}><RefreshCw size={10} /> Regenerate</button>
            <button onClick={() => navigator.clipboard.writeText(kikoRec)} style={{ padding: '6px 12px', borderRadius: 8, fontSize: 12, fontWeight: 400, border: '0.5px solid rgba(238,238,238,0.06)', background: 'transparent', color: T.textTertiary, cursor: 'pointer', fontFamily: T.font }}>Copy</button>
          </div>
        )}
      </div>
    </div>
